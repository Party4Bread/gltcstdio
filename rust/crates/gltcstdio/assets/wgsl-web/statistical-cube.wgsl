...
fn statisticalCube(pos: vec2<f32>, outPos: vec2<f32>, intensity_2: f32, model3DTransform: mat4x4<f32>, sourceDim: vec2<f32>) -> vec4<f32> {
    ...
    loop {
        {
            ...
            let _e274 = p;
            let _e285 = textureSampleLevel(t_source, samp, ((vec2<f32>((_e266.x / _e271.x), _e274.y) / vec2(2f)) + vec2(0.5f)), 0f);
            color_2 = _e285;
            let _e286 = intensity_3;
            let _e287 = color_2;
            let _e288 = height(_e286, _e287);
            h = _e288;
            let _e289 = p;
            let _e291 = h;
            dz = (_e289.z - _e291);
            ...
            let _e305 = dz;
            let _e307 = prevDz;
            stop = ((_e299 == 0f) || ((_e302 != _e303) && (sign(_e305) == -(sign(_e307)))));
        }
        let _e313 = k;
        let _e314 = k2_;
        let _e316 = stop;
        if !(((_e313 <= _e314) && !(_e316))) {
            break;
        }
    }
    let _e320 = stop;
    ...
    let _e337 = k2_;
    let _e338 = k1_;
    return vec4<f32>((_e326 - _e327), _e333, ((_e334 - _e335) / (_e337 - _e338)), 1f);
}
