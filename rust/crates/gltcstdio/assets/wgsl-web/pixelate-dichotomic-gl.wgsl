...
fn pixelateDichotomic(pos: vec2<f32>, outPos: vec2<f32>, sourceDim: vec2<f32>, modelTransform: mat3x3<f32>, randomSeed: f32, regularity: f32, thickness: f32, color1_: vec4<f32>, paletteDim: vec2<f32>) -> vec4<f32> {
    ...
    let _e240 = pos_1;
    let _e250 = textureSampleLevel(t_source, samp, ((vec2<f32>((_e233.x / _e237.x), _e240.y) / vec2(2f)) + vec2(0.5f)), 0f);
    col = _e250;
    let _e253 = border;
    if _e253 {
        {
            ...
            let _e256 = color1_1;
            let _e258 = color1_1;
            let _e261 = mix(_e254.xyz, _e256.xyz, vec3(_e258.w));
            let _e262 = col;
            outCol = vec4<f32>(_e261.x, _e261.y, _e261.z, _e262.w);
        }
    } else {
        {
            let _e268 = rect;
            ...
            let _e281 = rect;
            let _e283 = rect;
            let _e297 = textureSampleLevel(t_source, samp, ((vec2<f32>((((_e268.xy + _e270.zw) * 0.5f).x / _e278.x), ((_e281.xy + _e283.zw) * 0.5f).y) / vec2(2f)) + vec2(0.5f)), 0f);
            outCol = _e297;
            let _e298 = paletteDim_1;
            n = i32(_e298.x);
            let _e302 = n;
            doQuantize = (_e302 > 1i);
            let _e306 = n;
            if (_e306 == 1i) {
                let _e313 = textureLoad(t_palette, vec2<i32>(0i, 0i), 0i);
                doQuantize = (_e313.w > 0.5f);
            }
            let _e317 = doQuantize;
            if _e317 {
                {
                    let _e320 = outCol;
                    bestColor = _e320;
                    loop {
                        ...
                        {
                            let _e331 = i_1;
                            let _e335 = textureLoad(t_palette, vec2<i32>(_e331, 0i), 0i);
                            target_ = _e335;
                            let _e337 = outCol;
                            let _e338 = target_;
                            dist = length((_e337 - _e338));
                            let _e342 = dist;
                            let _e343 = minDist;
                            if (_e342 < _e343) {
                                {
