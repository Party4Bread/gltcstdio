...
fn dichotomicTiles(uv: vec2<f32>, outPos: vec2<f32>, source1Dim: vec2<f32>, source2Dim: vec2<f32>, source3Dim: vec2<f32>, variability: f32, randomSeed: f32, colorBkg: vec4<f32>, color: vec4<f32>, thickness: f32, modelTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    if (_e488 == 0i) {
        ...
        let _e498 = X_1;
        let _e508 = textureSampleLevel(t_source1_, samp, ((vec2<f32>((_e491.x / _e495.x), _e498.y) / vec2(2f)) + vec2(0.5f)), 0f);
        local_4 = _e508;
    } else {
        let _e509 = k_4;
        if (_e509 == 1i) {
            let _e512 = X_1;
            let _e516 = global.U[0];
            let _e519 = X_1;
            let _e529 = textureSampleLevel(t_source2_, samp, ((vec2<f32>((_e512.x / _e516.x), _e519.y) / vec2(2f)) + vec2(0.5f)), 0f);
            local_3 = _e529;
        } else {
            let _e530 = X_1;
            let _e534 = global.U[0];
            let _e537 = X_1;
            let _e547 = textureSampleLevel(t_source3_, samp, ((vec2<f32>((_e530.x / _e534.x), _e537.y) / vec2(2f)) + vec2(0.5f)), 0f);
            local_3 = _e547;
        }
        let _e549 = local_3;
        local_4 = _e549;
