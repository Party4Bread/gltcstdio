...
fn progressiveScaling(uv: vec2<f32>, outPos: vec2<f32>, sourceDim: vec2<f32>, mode: i32, backgroundMode: i32, balance: f32, power: f32, offset: f32, colorScheme: f32, texTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    col3_ = _e385;
    let _e387 = texTransform_1;
    let _e389 = uv_1;
    let _e390 = tf(_naga_inverse_3x3_f32(_e387), _e389);
    uv_1 = _e390;
    let _e391 = uv_1;
    if (fract(_e391.x) < 0.5f) {
        local_3 = 1f;
    } else {
        local_3 = 0f;
    }
    ...
    let _e405 = r;
    let _e406 = g;
    let _e408 = vec3<f32>(_e405, _e406, 0.5f);
    col2_ = vec4<f32>(_e408.x, _e408.y, _e408.z, 1f);
    let _e415 = g;
    if (_e415 > 0.5f) {
        local_4 = 1f;
    } else {
    ...
    let _e421 = local_4;
    let _e422 = vec3(_e421);
    col4_ = vec4<f32>(_e422.x, _e422.y, _e422.z, 1f);
    let _e429 = kCol;
    if (_e429 <= 0.5f) {
        let _e432 = uv_1;
        let _e436 = kCol;
        let _e439 = uv_1;
        if ((_e432.x - 0.5f) > ((_e436 * 2f) * (_e439.y - 0.5f))) {
            local_5 = 0f;
        } else {
            local_5 = 1f;
        }
        ...
        let _e459 = uv_1;
        let _e461 = kCol;
        local_6 = ((_e449 - 0.5f) + (0.25f * ((_e453.y / (_e455 - 0.5f)) - (_e459.x / (_e461 - 0.5f)))));
    }
    let _e469 = local_6;
    let _e470 = vec3(_e469);
    colSlopedThenGrad = vec4<f32>(_e470.x, _e470.y, _e470.z, 1f);
    let _e477 = uv_1;
    let _e484 = uv_1;
    if ((abs((_e477.x - 0.5f)) > 0.4f) || (abs((_e484.y - 0.5f)) > 0.4f)) {
    ...
    let _e499 = local_7;
    colBorder = _e499;
    let _e502 = colorScheme_1;
    if (_e502 < 0.2f) {
        let _e505 = col3_;
        let _e506 = colBorder;
        let _e507 = kCol;
        resCol = mix(_e505, _e506, vec4(_e507));
    } else {
        let _e510 = colorScheme_1;
        if (_e510 < 0.4f) {
            let _e513 = colBorder;
            let _e514 = col1_;
            let _e515 = kCol;
            resCol = mix(_e513, _e514, vec4(_e515));
        } else {
            let _e518 = colorScheme_1;
            if (_e518 < 0.6f) {
                let _e521 = col1_;
                let _e522 = col2_;
                let _e523 = kCol;
                resCol = mix(_e521, _e522, vec4(_e523));
            } else {
                let _e526 = colorScheme_1;
                if (_e526 < 0.8f) {
                    let _e529 = col2_;
                    let _e530 = col4_;
                    let _e531 = kCol;
                    resCol = mix(_e529, _e530, vec4(_e531));
                } else {
                    let _e534 = colSlopedThenGrad;
                    resCol = _e534;
                }
            }
        }
    ...
    let _e535 = inside;
    let _e536 = backgroundMode_1;
    if (_e535 || (_e536 <= 1i)) {
        let _e540 = resCol;
        return _e540;
    } else {
        let _e541 = backgroundMode_1;
        if (_e541 == 2i) {
            return vec4<f32>(0f, 0f, 0f, 1f);
        } else {
            let _e549 = backgroundMode_1;
            if (_e549 == 3i) {
                return vec4(1f);
            } else {
                let _e554 = outPos_1;
                let _e558 = global.U[0];
                let _e561 = outPos_1;
                let _e570 = _mirror_wrap(((vec2<f32>((_e554.x / _e558.x), _e561.y) / vec2(2f)) + vec2(0.5f)));
                let _e572 = textureSampleLevel(t_source, samp, _e570, 0f);
                return _e572;
            }
        }
    }
