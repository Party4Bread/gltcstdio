...
fn overlay(uv: vec2<f32>, outPos: vec2<f32>, blendMode: i32, intensity: f32, thickness: f32, shadows: f32, color: vec4<f32>, source2Dim: vec2<f32>, modelTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    let _e29 = global.U[0];
    let _e32 = uv_1;
    let _e42 = textureSampleLevel(t_source1_, samp, ((vec2<f32>((_e25.x / _e29.x), _e32.y) / vec2(2f)) + vec2(0.5f)), 0f);
    bkgColor = _e42;
    let _e44 = modelTransform_1;
    let _e46 = uv_1;
    let _e47 = tf(_naga_inverse_3x3_f32(_e44), _e46);
    u_4 = _e47;
    let _e49 = source2Dim_1;
    let _e51 = source2Dim_1;
    ratio2_ = (_e49.x / _e51.y);
    let _e55 = ratio2_;
    let _e56 = thickness_1;
    let _e61 = thickness_1;
    borderDim = vec2<f32>((_e55 + (_e56 * 0.3f)), (1f + (_e61 * 0.3f)));
    let _e67 = u_4;
    let _e70 = ratio2_;
    let _e72 = u_4;
    if ((abs(_e67.x) <= _e70) && (abs(_e72.y) <= 1f)) {
        {
            let _e78 = u_4;
            let _e82 = global.U[0];
            let _e85 = u_4;
            let _e95 = textureSampleLevel(t_source2_, samp, ((vec2<f32>((_e78.x / _e82.x), _e85.y) / vec2(2f)) + vec2(0.5f)), 0f);
            overColor = _e95;
            let _e97 = blendMode_1;
            let _e98 = bkgColor;
            let _e99 = overColor;
            let _e100 = blend(_e97, _e98, _e99);
            blended = _e100;
            let _e102 = bkgColor;
            let _e103 = blended;
            let _e104 = intensity_1;
            mixed = mix(_e102, _e103, vec4(_e104));
            let _e108 = bkgColor;
            let _e109 = mixed;
            let _e110 = mergeColor(_e108, _e109);
            return _e110;
        }
    } else {
        let _e111 = u_4;
        ...
        let _e117 = u_4;
        let _e120 = borderDim;
        if ((abs(_e111.x) <= _e114.x) && (abs(_e117.y) <= _e120.y)) {
            {
                let _e124 = color_1;
                return _e124;
            }
        } else {
            {
                ...
                let _e129 = u_4;
                let _e130 = borderDim;
                let _e131 = sdRectangle(_e129, _e130);
                d = _e131;
                let _e133 = shadows_1;
                let _e137 = d;
                s = (smoothstep((_e133 * 0.6f), 0f, _e137) * 0.5f);
                let _e142 = bkgColor;
                let _e146 = s;
                let _e148 = mergeColor(_e142, vec4<f32>(0f, 0f, 0f, _e146));
                return _e148;
            }
        }
    }
