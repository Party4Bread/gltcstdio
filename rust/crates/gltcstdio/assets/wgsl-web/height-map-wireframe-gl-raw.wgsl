...
fn heightMapWireframeGl(pos: vec2<f32>, outPos: vec2<f32>, intensity_2: f32, rezolution: i32, model3DTransform: mat4x4<f32>, sourceDim: vec2<f32>, sourceElevationDim: vec2<f32>, sourceBkg_specified: i32, sourceElevation_specified: i32, thickness_4: f32, glow_4: f32, colorLines: vec4<f32>) -> vec4<f32> {
    ...
    if (_e243 > _e244) {
        ...
        if (_e246 == 1i) {
            ...
            let _e256 = outPos_1;
            let _e266 = textureSampleLevel(t_sourceBkg, samp, ((vec2<f32>((_e249.x / _e253.x), _e256.y) / vec2(2f)) + vec2(0.5f)), 0f);
            local_2 = _e266;
        } else {
            let _e267 = outPos_1;
    ...
    let _e287 = k1_;
    k = _e287;
    let _e289 = cameraPos_4;
    let _e290 = k;
    let _e291 = dir;
    p = (_e289 + (_e290 * _e291));
    let _e295 = ratio;
    let _e298 = rezolution_1;
    strideX = ((_e295 * 2f) / f32(_e298));
    let _e303 = strideX;
    countY = floor(((2f / _e303) + 0.5f));
    let _e310 = countY;
    strideY = (2f / _e310);
    let _e316 = p;
    let _e320 = strideY;
    yPos = ((_e316.y + 1f) / _e320);
    let _e323 = yPos;
    yIndex = floor((_e323 + 0.5f));
    let _e328 = yPos;
    let _e329 = yIndex;
    let _e330 = close(_e328, _e329);
    if _e330 {
        {
            ...
            if _e332 {
                ...
                let _e341 = p;
                let _e352 = textureSampleLevel(t_sourceElevation, samp, ((vec2<f32>((_e333.x / _e338.x), _e341.y) / vec2(2f)) + vec2(0.5f)), 0f);
                local_3 = _e352;
            } else {
                let _e353 = p;
                let _e358 = global.U[0];
                let _e361 = p;
                let _e372 = textureSampleLevel(t_source, samp, ((vec2<f32>((_e353.x / _e358.x), _e361.y) / vec2(2f)) + vec2(0.5f)), 0f);
                local_3 = _e372;
            }
            let _e374 = local_3;
            let _e375 = height(_e331, _e374);
            _h = _e375;
            let _e376 = intersected;
            let _e377 = p;
            let _e378 = cameraPos_4;
            let _e379 = cameraDir_4;
            let _e380 = _h;
            let _e381 = thickness_5;
            let _e382 = glow_5;
            let _e383 = intersectY_hmwgl(_e377, _e378, _e379, _e380, _e381, _e382);
            intersected = (_e376 + _e383);
        }
    }
    let _e385 = dir;
    if (_e385.y != 0f) {
        {
            ...
            if (sign(_e389.y) > 0f) {
                let _e394 = yPos;
                let _e396 = yPos;
                local_4 = (ceil(_e394) - _e396);
            } else {
                let _e398 = yPos;
                let _e400 = yPos;
                local_4 = (floor(_e398) - _e400);
            }
            let _e403 = local_4;
            let _e404 = strideY;
            advanceY = (_e403 * _e404);
            let _e407 = advanceY;
            let _e408 = dir;
            deltaK = (_e407 / _e408.y);
            let _e412 = k;
            let _e413 = deltaK;
            k = (_e412 + _e413);
            let _e415 = p;
            ...
            let _e420 = dir;
            let _e423 = strideY;
            deltaY = (sign(_e420.y) * _e423);
            let _e426 = deltaY;
            let _e427 = dir;
            deltaK = (_e426 / _e427.y);
            loop {
                let _e432 = p;
                let _e437 = k;
                let _e438 = k2_;
                let _e441 = maxIter;
                if !((((abs(_e432.y) <= 1f) && (_e437 <= _e438)) && (_e441 > 0i))) {
                    break;
                }
                {
                    let _e446 = intensity_3;
                    let _e447 = heightMap;
                    if _e447 {
                        let _e448 = p;
                        let _e453 = global.U[0];
                        let _e456 = p;
                        let _e467 = textureSampleLevel(t_sourceElevation, samp, ((vec2<f32>((_e448.x / _e453.x), _e456.y) / vec2(2f)) + vec2(0.5f)), 0f);
                        local_5 = _e467;
                    } else {
                        let _e468 = p;
                        let _e473 = global.U[0];
                        let _e476 = p;
                        let _e487 = textureSampleLevel(t_source, samp, ((vec2<f32>((_e468.x / _e473.x), _e476.y) / vec2(2f)) + vec2(0.5f)), 0f);
                        local_5 = _e487;
                    }
                    let _e489 = local_5;
                    let _e490 = height(_e446, _e489);
                    _h = _e490;
                    let _e491 = intersected;
                    let _e492 = p;
                    let _e493 = cameraPos_4;
                    let _e494 = cameraDir_4;
                    let _e495 = _h;
                    let _e496 = thickness_5;
                    let _e497 = glow_5;
                    let _e498 = intersectY_hmwgl(_e492, _e493, _e494, _e495, _e496, _e497);
                    intersected = (_e491 + _e498);
                    let _e500 = intersected;
                    if (_e500 >= 1f) {
                        break;
                    }
                    let _e503 = k;
    ...
    let _e514 = k1_;
    k = _e514;
    let _e515 = cameraPos_4;
    let _e516 = k;
    let _e517 = dir;
    p = (_e515 + (_e516 * _e517));
    let _e520 = p;
    let _e524 = strideX;
    xPos = ((_e520.x + 1f) / _e524);
    let _e527 = xPos;
    xIndex = floor((_e527 + 0.5f));
    let _e532 = xPos;
    let _e533 = xIndex;
    let _e534 = close(_e532, _e533);
    if _e534 {
        {
            ...
            if _e536 {
                let _e537 = p;
                let _e542 = global.U[0];
                let _e545 = p;
                let _e556 = textureSampleLevel(t_sourceElevation, samp, ((vec2<f32>((_e537.x / _e542.x), _e545.y) / vec2(2f)) + vec2(0.5f)), 0f);
                local_6 = _e556;
            } else {
                let _e557 = p;
            ...
            let _e578 = local_6;
            let _e579 = height(_e535, _e578);
            _h = _e579;
            let _e580 = intersected;
            let _e581 = p;
            let _e582 = cameraPos_4;
            let _e583 = cameraDir_4;
            let _e584 = _h;
            let _e585 = thickness_5;
            let _e586 = glow_5;
            let _e587 = intersectX_hmwgl(_e581, _e582, _e583, _e584, _e585, _e586);
            intersected = (_e580 + _e587);
        }
    }
    let _e589 = dir;
    if (_e589.x != 0f) {
        {
            ...
            if (sign(_e593.x) > 0f) {
                let _e598 = xPos;
                let _e600 = xPos;
                local_7 = (ceil(_e598) - _e600);
            } else {
                let _e602 = xPos;
                let _e604 = xPos;
                local_7 = (floor(_e602) - _e604);
            }
            let _e607 = local_7;
            let _e608 = strideX;
            advanceX = (_e607 * _e608);
            let _e611 = advanceX;
            let _e612 = dir;
            deltaK_1 = (_e611 / _e612.x);
            let _e616 = k;
            let _e617 = deltaK_1;
            k = (_e616 + _e617);
            let _e619 = p;
            ...
            let _e624 = dir;
            let _e627 = strideX;
            deltaX = (sign(_e624.x) * _e627);
            let _e630 = deltaX;
            let _e631 = dir;
            deltaK_1 = (_e630 / _e631.x);
            loop {
                let _e636 = p;
                let _e639 = ratio;
                let _e641 = k;
                let _e642 = k2_;
                let _e645 = maxIter_1;
                if !((((abs(_e636.x) <= _e639) && (_e641 <= _e642)) && (_e645 > 0i))) {
                    break;
                }
                {
                    let _e650 = intensity_3;
                    let _e651 = heightMap;
                    if _e651 {
                        let _e652 = p;
                        let _e657 = global.U[0];
                        let _e660 = p;
                        let _e671 = textureSampleLevel(t_sourceElevation, samp, ((vec2<f32>((_e652.x / _e657.x), _e660.y) / vec2(2f)) + vec2(0.5f)), 0f);
                        local_8 = _e671;
                    } else {
                        let _e672 = p;
                        let _e677 = global.U[0];
                        let _e680 = p;
                        let _e691 = textureSampleLevel(t_source, samp, ((vec2<f32>((_e672.x / _e677.x), _e680.y) / vec2(2f)) + vec2(0.5f)), 0f);
                        local_8 = _e691;
                    }
                    let _e693 = local_8;
                    let _e694 = height(_e650, _e693);
                    _h = _e694;
                    let _e695 = intersected;
                    let _e696 = p;
                    let _e697 = cameraPos_4;
                    let _e698 = cameraDir_4;
                    let _e699 = _h;
                    let _e700 = thickness_5;
                    let _e701 = glow_5;
                    let _e702 = intersectX_hmwgl(_e696, _e697, _e698, _e699, _e700, _e701);
                    intersected = (_e695 + _e702);
                    let _e704 = intersected;
                    if (_e704 >= 1f) {
                        break;
                    }
                    let _e707 = k;
    ...
    let _e718 = colorLines_1;
    wireColor = _e718;
    let _e720 = sourceBkg_specified_1;
    if (_e720 == 1i) {
        let _e723 = outPos_1;
        let _e727 = global.U[0];
        let _e730 = outPos_1;
        let _e740 = textureSampleLevel(t_sourceBkg, samp, ((vec2<f32>((_e723.x / _e727.x), _e730.y) / vec2(2f)) + vec2(0.5f)), 0f);
        local_9 = _e740;
    } else {
        let _e741 = outPos_1;
        let _e745 = global.U[0];
        let _e748 = outPos_1;
        let _e758 = textureSampleLevel(t_source, samp, ((vec2<f32>((_e741.x / _e745.x), _e748.y) / vec2(2f)) + vec2(0.5f)), 0f);
        local_9 = _e758;
    }
    let _e760 = local_9;
    let _e761 = wireColor;
    let _e762 = intersected;
    return mix(_e760, _e761, vec4(clamp(_e762, 0f, 1f)));
}
