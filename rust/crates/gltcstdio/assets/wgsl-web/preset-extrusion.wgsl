...
fn topography(pos: vec2<f32>, outPos: vec2<f32>, intensity: f32, count: i32, overlap: f32, mode: i32, sourceBkg_specified: i32, colorFog: vec4<f32>, model3DTransform: mat4x4<f32>, sourceDim: vec2<f32>) -> vec4<f32> {
    ...
    loop {
        ...
        }
        {
                    ...
                    if (!(_e195) || ((abs(_e197.x) < _e200) && (abs(_e202.y) < 1f))) {
                        {
                            ...
                            let _e227 = textureSampleLevel(t_source, samp, _e225, 0f);
                            sampleCol = _e227;
                            let _e229 = sampleCol;
                            let _e231 = luma(_e229.xyz);
                            lum = _e231;
                            let _e233 = layerOffset;
                            let _e234 = i;
                            layerStart = (_e233 * f32(_e234));
                            let _e238 = layerStart;
                            let _e239 = layerSize;
                            layerEnd = (_e238 + _e239);
                            let _e242 = lum;
                            let _e243 = layerStart;
                            let _e245 = lum;
                            let _e246 = layerEnd;
                            if ((_e242 >= _e243) && (_e245 <= _e246)) {
                                {
                                    let _e249 = dir;
                                    let _e250 = k;
                                    let _e252 = cameraPos;
                                    intersection = ((_e249 * _e250) + _e252);
                                    let _e255 = cameraPos;
                                    let _e256 = intersection;
                                    kFog = length((_e255 - _e256));
                                    let _e259 = col;
                                    let _e260 = sampleCol;
                                    let _e261 = mergeColorOpacifying(_e259, _e260);
                                    col = _e261;
                                    let _e262 = col;
                                    if (_e262.w == 1f) {
                                        break;
            ...
            let _e274 = iterDir;
            let _e276 = i;
            if ((_e269 && (_e270 >= _e271)) || (!(_e274) && (_e276 < 0i))) {
                break;
            }
        }
    }
    ...
    if (_e281.w < 1f) {
        {
            let _e285 = colorFog_1;
            if (_e285.w != 0f) {
                let _e289 = colorFog_1;
                let _e290 = _e289.xyz;
                local_3 = vec4<f32>(_e290.x, _e290.y, _e290.z, 1f);
            } else {
                let _e296 = sourceBkg_specified_1;
                if (_e296 == 1i) {
                    let _e299 = outPos_1;
                    let _e303 = global.U[0];
                    let _e306 = outPos_1;
                    let _e315 = _mirror_wrap(((vec2<f32>((_e299.x / _e303.x), _e306.y) / vec2(2f)) + vec2(0.5f)));
                    let _e317 = textureSampleLevel(t_sourceBkg, samp, _e315, 0f);
                    local_2 = _e317;
                } else {
    ...
    let _e331 = colorFog_1;
    if (_e331.w != 0f) {
        {
            let _e337 = colorFog_1;
            nearDist = (2f * (1f - _e337.w));
            let _e343 = nearDist;
            farDist = (2f * _e343);
            let _e346 = nearDist;
            let _e347 = farDist;
            let _e348 = kFog;
            kFog = smoothstep(_e346, _e347, _e348);
            let _e350 = col;
            let _e352 = col;
            let _e354 = colorFog_1;
            let _e356 = kFog;
            let _e358 = mix(_e352.xyz, _e354.xyz, vec3(_e356));
            col.x = _e358.x;
            col.y = _e358.y;
            col.z = _e358.z;
        }
