...
fn noiseCombine(pos: vec2<f32>, outPos: vec2<f32>, coverage: f32, aspectRatio_2: f32, octaves: i32, randomSeed: f32, shapeAspectRatio: f32, modelTransform: mat3x3<f32>, source1Dim: vec2<f32>, source2Dim: vec2<f32>, viewTransform1_: mat3x3<f32>, viewTransform2_: mat3x3<f32>) -> vec4<f32> {
    ...
    if (_e168 < _e169) {
        ...
        let _e198 = textureSampleLevel(t_source1_, samp, ((vec2<f32>((_e176.x / _e180.x), _e188.y) / vec2(2f)) + vec2(0.5f)), 0f);
        return _e198;
    } else {
        let _e199 = fit2_;
        let _e200 = viewTransform2_1;
        let _e203 = pos_1;
        let _e204 = tf((_e199 * _naga_inverse_3x3_f32(_e200)), _e203);
        let _e208 = global.U[0];
        let _e211 = fit2_;
        let _e212 = viewTransform2_1;
        let _e215 = pos_1;
        let _e216 = tf((_e211 * _naga_inverse_3x3_f32(_e212)), _e215);
        let _e226 = textureSampleLevel(t_source2_, samp, ((vec2<f32>((_e204.x / _e208.x), _e216.y) / vec2(2f)) + vec2(0.5f)), 0f);
        return _e226;
    }
}
