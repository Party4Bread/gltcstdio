...
fn sdStar(u: vec2<f32>, spikeCount: i32, r_1: f32, m_2: f32) -> f32 {
    var u_1: vec2<f32>;
...
fn basicInterpolatedShapes(uv: vec2<f32>, outPos: vec2<f32>, insideImage_specified: i32, spikeCount_2: i32, radius: f32, shadows: f32, roundness: f32, multiplier: f32, colorOutline: vec4<f32>, outlineThickness: f32, brightness_2: f32, contrast_2: f32, saturation_2: f32, hue_2: f32, colorIn: vec4<f32>, colorOut: vec4<f32>, colorShadow: vec4<f32>, colorGlow: vec4<f32>, insideLock: i32, modelTransform: mat3x3<f32>, insideTransform: mat3x3<f32>, shadowTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    let _e53 = uv_1;
    let _e54 = tf(_naga_inverse_3x3_f32(_e51), _e53);
    u_4 = _e54;
    let _e58 = u_4;
    let _e60 = u_4;
    let _e64 = spikeCount_3;
    let _e67 = radius_1;
    let _e68 = spikeCount_3;
    let _e74 = sdStar(vec2<f32>(_e58.x, -(_e60.y)), _e64, 0.75f, (2f + (_e67 * (f32(_e68) - 2f))));
    d = _e74;
    let _e75 = d;
    let _e76 = multiplier_1;
    let _e78 = roundness_1;
    d = ((_e75 * _e76) - _e78);
    let _e85 = uv_1;
    v = _e85;
    let _e87 = d;
    ...
    if _e91 {
        {
            ...
            if (_e92 < 0f) {
                {
                    ...
                    saveD = _e99;
                    let _e101 = d;
                    let _e102 = multiplier_1;
                    let _e104 = roundness_1;
                    d = ((_e101 * _e102) - _e104);
                    let _e106 = u_4;
                    let _e108 = u_4;
                    let _e112 = spikeCount_3;
                    let _e115 = radius_1;
                    let _e116 = spikeCount_3;
                    let _e122 = sdStar(vec2<f32>(_e106.x, -(_e108.y)), _e112, 0.75f, (2f + (_e115 * (f32(_e116) - 2f))));
                    d = _e122;
                    let _e124 = shadows_1;
                    let _e126 = d;
                    shadow = (0.7f * smoothstep(_e124, 0f, _e126));
                    let _e129 = saveD;
                    d = _e129;
                }
            }
            let _e130 = colorIn_1;
            tint_4 = _e130;
            let _e131 = insideLock_1;
            if (_e131 == 0i) {
                let _e134 = insideTransform_1;
                local_8 = _e134;
            } else {
                let _e135 = modelTransform_1;
                let _e136 = insideTransform_1;
                local_8 = (_e135 * _e136);
            }
            let _e139 = local_8;
            iTransform = _e139;
            let _e141 = iTransform;
            let _e143 = uv_1;
            let _e144 = tf(_naga_inverse_3x3_f32(_e141), _e143);
            v = _e144;
        }
    } else {
        {
            ...
            if (_e145 > 0f) {
                {
                    ...
                    saveD_1 = _e152;
                    let _e154 = d;
                    let _e155 = multiplier_1;
                    let _e157 = roundness_1;
                    d = ((_e154 * _e155) - _e157);
                    let _e159 = u_4;
                    let _e161 = u_4;
                    let _e165 = spikeCount_3;
                    let _e168 = radius_1;
                    let _e169 = spikeCount_3;
                    let _e175 = sdStar(vec2<f32>(_e159.x, -(_e161.y)), _e165, 0.75f, (2f + (_e168 * (f32(_e169) - 2f))));
                    d = _e175;
                    let _e177 = shadows_1;
                    let _e179 = d;
                    shadow = (0.7f * smoothstep(_e177, 0f, _e179));
                    let _e182 = saveD_1;
                    d = _e182;
                }
            }
            let _e183 = colorOut_1;
            tint_4 = _e183;
        }
    }
    let _e184 = insideImage_specified_1;
    let _e187 = inside;
    if ((_e184 == 1i) && _e187) {
        let _e189 = v;
        let _e193 = global.U[0];
        let _e196 = v;
        let _e205 = _mirror_wrap(((vec2<f32>((_e189.x / _e193.x), _e196.y) / vec2(2f)) + vec2(0.5f)));
        let _e207 = textureSampleLevel(t_insideImage, samp, _e205, 0f);
        local_9 = _e207;
    } else {
        let _e208 = v;
        let _e212 = global.U[0];
        let _e215 = v;
        let _e224 = _mirror_wrap(((vec2<f32>((_e208.x / _e212.x), _e215.y) / vec2(2f)) + vec2(0.5f)));
        let _e226 = textureSampleLevel(t_source, samp, _e224, 0f);
        local_9 = _e226;
    }
    let _e228 = local_9;
    sColor = _e228;
    ...
    if _e232 {
        let _e233 = color;
        let _e234 = brightness_3;
        let _e235 = contrast_3;
        let _e238 = saturation_3;
        let _e239 = hue_3;
        let _e242 = adjustColorHSLuv(_e233, _e234, _e235, 0f, 0f, _e238, _e239, vec4(0f));
        color = _e242;
    }
    let _e243 = colorGlow_1;
    if (_e243.w != 0f) {
        let _e247 = colorGlow_1;
        let _e251 = d;
        let _e254 = ((_e247.xyz * 0.01f) / vec3(abs(_e251)));
        let _e256 = colorGlow_1;
        let _e260 = d;
        local_10 = vec4<f32>(_e254.x, _e254.y, _e254.z, min(1f, ((_e256.w * 0.01f) / abs(_e260))));
    } else {
        local_10 = vec4(0f);
    }
    let _e271 = local_10;
    ...
    let _e273 = color;
    let _e274 = tint_4;
    let _e275 = mergeColor(_e273, _e274);
    let _e276 = colorShadow_1;
    let _e277 = _e276.xyz;
    let _e278 = colorShadow_1;
    let _e280 = shadow;
    let _e286 = mergeColor(_e275, vec4<f32>(_e277.x, _e277.y, _e277.z, (_e278.w * _e280)));
    let _e287 = glow_2;
    let _e288 = mergeGlow(_e286, _e287);
    color = _e288;
    let _e289 = d;
    let _e291 = outlineThickness_1;
    if (abs(_e289) < (_e291 * 0.5f)) {
        {
            let _e295 = color;
            let _e296 = colorOutline_1;
...
fn main_1() {
    ...
    let _e125 = global.U[20];
    let _e128 = global.U[21];
    let _e133 = global.U[22];
    let _e134 = _e133.xyz;
    let _e137 = global.U[23];
    let _e138 = _e137.xyz;
    let _e141 = global.U[24];
    let _e142 = _e141.xyz;
    let _e158 = global.U[25];
    let _e159 = _e158.xyz;
    let _e162 = global.U[26];
    let _e163 = _e162.xyz;
    let _e166 = global.U[27];
    let _e167 = _e166.xyz;
    let _e183 = global.U[28];
    let _e184 = _e183.xyz;
    let _e187 = global.U[29];
    let _e188 = _e187.xyz;
    let _e191 = global.U[30];
    let _e192 = _e191.xyz;
    let _e206 = basicInterpolatedShapes((_naga_inverse_3x3_f32(mat3x3<f32>(vec3<f32>(_e10.x, _e10.y, _e10.z), vec3<f32>(_e14.x, _e14.y, _e14.z), vec3<f32>(_e18.x, _e18.y, _e18.z))) * vec3<f32>(_e45.x, _e45.y, 1f)).xy, (((_e52 - vec2(0.5f)) * 2f) * vec2<f32>(_e60.x, 1f)), i32(_e67.x), i32(_e72.x), _e77.x, _e81.x, _e85.x, _e89.x, _e93, _e96.x, _e100.x, _e104.x, _e108.x, _e112.x, _e116, _e119, _e122, _e125, i32(_e128.x), mat3x3<f32>(vec3<f32>(_e134.x, _e134.y, _e134.z), vec3<f32>(_e138.x, _e138.y, _e138.z), vec3<f32>(_e142.x, _e142.y, _e142.z)), mat3x3<f32>(vec3<f32>(_e159.x, _e159.y, _e159.z), vec3<f32>(_e163.x, _e163.y, _e163.z), vec3<f32>(_e167.x, _e167.y, _e167.z)), mat3x3<f32>(vec3<f32>(_e184.x, _e184.y, _e184.z), vec3<f32>(_e188.x, _e188.y, _e188.z), vec3<f32>(_e192.x, _e192.y, _e192.z)));
    fragColor = _e206;
    return;
}
