...
fn dichotomicBreak(uv: vec2<f32>, outPos: vec2<f32>, sourceDim: vec2<f32>, intensity_2: f32, iterations: i32, variability: f32, randomSeed: f32, color: vec4<f32>, thickness: f32, mode_2: i32, modelTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    let _e314 = uv_1;
    let _e324 = textureSampleLevel(t_source, samp, ((vec2<f32>((_e307.x / _e311.x), _e314.y) / vec2(2f)) + vec2(0.5f)), 0f);
    col = _e324;
    let _e326 = border;
    if _e326 {
        let _e327 = col;
        let _e329 = color_1;
        let _e331 = color_1;
        let _e334 = mix(_e327.xyz, _e329.xyz, vec3(_e331.w));
        let _e335 = col;
        local = vec4<f32>(_e334.x, _e334.y, _e334.z, _e335.w);
    } else {
        let _e341 = p_2;
