    ...
    if (abs(_e131) <= _e133) {
        {
            ...
            let _e160 = textureSampleLevel(t_source, samp, ((vec2<f32>((_e141.x / _e145.x), _e150.y) / vec2(2f)) + vec2(0.5f)), 0f);
            return _e160;
        }
    } else {
        {
            let _e161 = xpp;
            let _e163 = blendStart;
            let _e165 = blend_1;
            k = ((abs(_e161) - _e163) / _e165);
            let _e168 = sx;
            let _e169 = sy;
            pos1_ = vec2<f32>(_e168, _e169);
            let _e172 = xp;
            if (_e172 >= 0f) {
                let _e175 = sx;
                let _e176 = blendedRatio;
                local_1 = (_e175 - (_e176 * 2f));
            } else {
                let _e180 = sx;
                let _e181 = blendedRatio;
                local_1 = (_e180 + (_e181 * 2f));
            }
            let _e186 = local_1;
            sx2_ = _e186;
            let _e188 = sx2_;
            let _e189 = sy;
            pos2_ = vec2<f32>(_e188, _e189);
            let _e192 = inverseTexTransform;
            let _e193 = pos1_;
            let _e194 = tf(_e192, _e193);
            let _e198 = global.U[0];
            let _e201 = inverseTexTransform;
            let _e202 = pos1_;
            let _e203 = tf(_e201, _e202);
            let _e213 = textureSampleLevel(t_source, samp, ((vec2<f32>((_e194.x / _e198.x), _e203.y) / vec2(2f)) + vec2(0.5f)), 0f);
            let _e214 = inverseTexTransform;
            let _e215 = pos2_;
            let _e216 = tf(_e214, _e215);
            let _e220 = global.U[0];
            let _e223 = inverseTexTransform;
            let _e224 = pos2_;
            let _e225 = tf(_e223, _e224);
            let _e235 = textureSampleLevel(t_source, samp, ((vec2<f32>((_e216.x / _e220.x), _e225.y) / vec2(2f)) + vec2(0.5f)), 0f);
            let _e236 = k;
            return mix(_e213, _e235, vec4(_e236));
        }
    }
}
