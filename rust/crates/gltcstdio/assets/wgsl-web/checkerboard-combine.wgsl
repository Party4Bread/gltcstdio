...
fn checkerboardCombine(pos: vec2<f32>, outPos: vec2<f32>, thickness: f32, borderColor: vec4<f32>, source1Dim: vec2<f32>, source2Dim: vec2<f32>, viewTransform1_: mat3x3<f32>, viewTransform2_: mat3x3<f32>) -> vec4<f32> {
    ...
    if (_e88 > 0f) {
        ...
        let _e118 = textureSampleLevel(t_source1_, samp, ((vec2<f32>((_e96.x / _e100.x), _e108.y) / vec2(2f)) + vec2(0.5f)), 0f);
        local = _e118;
    } else {
        let _e119 = fit2_;
        let _e120 = viewTransform2_1;
        let _e123 = v;
        let _e124 = tf((_e119 * _naga_inverse_3x3_f32(_e120)), _e123);
        let _e128 = global.U[0];
        let _e131 = fit2_;
        let _e132 = viewTransform2_1;
        let _e135 = v;
        let _e136 = tf((_e131 * _naga_inverse_3x3_f32(_e132)), _e135);
        let _e146 = textureSampleLevel(t_source2_, samp, ((vec2<f32>((_e124.x / _e128.x), _e136.y) / vec2(2f)) + vec2(0.5f)), 0f);
        local = _e146;
    }
    let _e148 = local;
    col = _e148;
