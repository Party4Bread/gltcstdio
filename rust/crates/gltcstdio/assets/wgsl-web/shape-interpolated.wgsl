...
fn basicInterpolatedShapes(uv: vec2<f32>, outPos: vec2<f32>, insideImage_specified: i32, shape: f32, shadows: f32, roundness: f32, multiplier: f32, colorOutline: vec4<f32>, outlineThickness: f32, brightness_2: f32, contrast_2: f32, saturation_2: f32, hue_2: f32, colorIn: vec4<f32>, colorOut: vec4<f32>, colorShadow: vec4<f32>, colorGlow: vec4<f32>, insideLock: i32, modelTransform: mat3x3<f32>, insideTransform: mat3x3<f32>, shadowTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    if ((_e242 == 1i) && _e245) {
        ...
        let _e265 = textureSampleLevel(t_insideImage, samp, _e263, 0f);
        local_10 = _e265;
    } else {
        let _e266 = v;
        let _e270 = global.U[0];
        let _e273 = v;
        let _e282 = _mirror_wrap(((vec2<f32>((_e266.x / _e270.x), _e273.y) / vec2(2f)) + vec2(0.5f)));
        let _e284 = textureSampleLevel(t_source, samp, _e282, 0f);
        local_10 = _e284;
    }
    let _e286 = local_10;
    sColor = _e286;
    ...
    if _e290 {
        let _e291 = color;
        let _e292 = brightness_3;
        let _e293 = contrast_3;
        let _e296 = saturation_3;
        let _e297 = hue_3;
        let _e300 = adjustColorHSLuv(_e291, _e292, _e293, 0f, 0f, _e296, _e297, vec4(0f));
        color = _e300;
    }
    let _e301 = colorGlow_1;
    if (_e301.w != 0f) {
        let _e305 = colorGlow_1;
        let _e309 = d;
        let _e312 = ((_e305.xyz * 0.01f) / vec3(abs(_e309)));
        let _e314 = colorGlow_1;
        let _e318 = d;
        local_11 = vec4<f32>(_e312.x, _e312.y, _e312.z, min(1f, ((_e314.w * 0.01f) / abs(_e318))));
    } else {
        local_11 = vec4(0f);
    }
    let _e329 = local_11;
    ...
    let _e335 = _e334.xyz;
    let _e336 = colorShadow_1;
    let _e338 = shadow;
    let _e344 = mergeColor(_e333, vec4<f32>(_e335.x, _e335.y, _e335.z, (_e336.w * _e338)));
    let _e345 = glow_2;
    let _e346 = mergeGlow(_e344, _e345);
    color = _e346;
    let _e347 = d;
    let _e349 = outlineThickness_1;
    if (abs(_e347) < (_e349 * 0.5f)) {
        {
            let _e353 = color;
            let _e354 = colorOutline_1;
