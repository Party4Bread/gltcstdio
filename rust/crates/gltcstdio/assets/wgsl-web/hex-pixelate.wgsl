    ...
    if (_e43.y < (_e45 * 0.5f)) {
        {
            ...
            let _e67 = textureSampleLevel(t_source, samp, _e65, 0f);
            col = _e67;
            let _e69 = col;
            let _e70 = color_1;
            let _e71 = mergeColor(_e69, _e70);
            return _e71;
        }
    } else {
        {
            ...
            if (_e72 >= 0f) {
                {
                    ...
                    let _e99 = l;
                    let _e102 = hex;
                    let _e115 = _mirror_wrap(((vec2<f32>(((_e81 + ((_e82 * _e83) * vec2<f32>(0f, _e86.y))).x / _e94.x), (_e97 + ((_e98 * _e99) * vec2<f32>(0f, _e102.y))).y) / vec2(2f)) + vec2(0.5f)));
                    let _e117 = textureSampleLevel(t_source, samp, _e115, 0f);
                    return _e117;
                }
            } else {
                {
                    let _e118 = u_2;
                    let _e119 = hexPolarCoords(_e118);
                    hex2_ = _e119;
                    let _e121 = hex;
                    let _e126 = (_e121.x + 0.5235988f);
                    ang = ((_e126 - (floor((_e126 / 1.0471976f)) * 1.0471976f)) - 0.5235988f);
                    let _e140 = ang;
                    k = (1f / cos(_e140));
                    ...
                    let _e152 = hex;
                    let _e155 = hex;
                    let _e161 = tf(_e144, (_e145.zw - (((_e147 * 0.5f) * _e150) * vec2<f32>(cos(_e152.x), sin(_e155.x)))));
                    v2_ = _e161;
                    let _e163 = v2_;
                    let _e167 = global.U[0];
                    let _e170 = v2_;
                    let _e179 = _mirror_wrap(((vec2<f32>((_e163.x / _e167.x), _e170.y) / vec2(2f)) + vec2(0.5f)));
                    let _e181 = textureSampleLevel(t_source, samp, _e179, 0f);
                    return _e181;
                }
            }
        }
