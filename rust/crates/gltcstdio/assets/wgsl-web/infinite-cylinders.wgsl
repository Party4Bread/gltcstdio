...
fn sdf(p: vec3<f32>) -> f32 {
    ...
    let _e9 = p_1;
    let _e11 = vec3(1f);
    qq = ((_e9 - (floor((_e9 / _e11)) * _e11)) - vec3(0.5f));
    let _e20 = p_1;
    fp = floor(_e20);
    let _e23 = fp;
    let _e25 = fp;
    let _e28 = fp;
    let _e30 = ((_e23.x + _e25.y) + _e28.z);
    m = (_e30 - (floor((_e30 / 3f)) * 3f));
    let _e37 = m;
    if (_e37 == 0f) {
        let _e41 = qq;
        let _e44 = qq;
        return (0.8f * (max(abs(_e41.x), length(_e44.zy)) - 0.2f));
    } else {
        let _e51 = m;
        if (_e51 == 1f) {
            let _e55 = qq;
            let _e58 = qq;
            return (0.8f * (max(abs(_e55.y), length(_e58.xz)) - 0.2f));
        } else {
            let _e66 = qq;
            let _e69 = qq;
            return (0.8f * (max(abs(_e66.z), length(_e69.xy)) - 0.2f));
        }
    }
}
