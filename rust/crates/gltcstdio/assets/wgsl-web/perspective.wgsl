            ...
            if (!(_e123) || ((abs(_e125.x) < _e128) && (abs(_e130.y) < 1f))) {
                {
                    ...
                    let _e144 = uv;
                    let _e154 = textureSampleLevel(t_source, samp, ((vec2<f32>((_e137.x / _e141.x), _e144.y) / vec2(2f)) + vec2(0.5f)), 0f);
                    col = _e154;
                    let _e155 = k;
                    d = abs(_e155);
                    let _e158 = d;
                    let _e161 = highFreqColor_1;
                    if ((_e158 > 1f) && (_e161.w > 0f)) {
                        {
                            let _e166 = d;
                            fog = ((abs(_e166) - 1f) * 0.2f);
                            let _e173 = col;
                            let _e174 = highFreqColor_1;
                            let _e175 = _e174.xyz;
                            let _e177 = highFreqColor_1;
                            let _e179 = fog;
                            let _e186 = mergeColor(_e173, vec4<f32>(_e175.x, _e175.y, _e175.z, min(1f, (_e177.w * _e179))));
                            col = _e186;
                        }
                    }
                }
