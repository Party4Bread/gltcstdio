...
fn basicInterpolatedShapes(uv: vec2<f32>, outPos: vec2<f32>, insideImage_specified: i32, shadows: f32, roundness: f32, multiplier: f32, colorOutline: vec4<f32>, outlineThickness: f32, brightness_2: f32, contrast_2: f32, saturation_2: f32, hue_2: f32, colorIn: vec4<f32>, colorOut: vec4<f32>, colorShadow: vec4<f32>, colorGlow: vec4<f32>, insideLock: i32, modelTransform: mat3x3<f32>, insideTransform: mat3x3<f32>, shadowTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    let _e54 = u_4;
    let _e56 = u_4;
    let _e69 = sdStar(vec2<f32>(_e54.x, -(_e56.y)), 24i, 0.75f, 22.460001f);
    d = _e69;
    let _e70 = d;
    let _e71 = multiplier_1;
    let _e73 = roundness_1;
    d = ((_e70 * _e71) - _e73);
    let _e80 = uv_1;
    v = _e80;
    let _e82 = d;
    ...
    if _e86 {
        {
            ...
            if (_e87 < 0f) {
                {
                    ...
                    saveD = _e94;
                    let _e96 = d;
                    let _e97 = multiplier_1;
                    let _e99 = roundness_1;
                    d = ((_e96 * _e97) - _e99);
                    let _e101 = u_4;
                    let _e103 = u_4;
                    let _e116 = sdStar(vec2<f32>(_e101.x, -(_e103.y)), 24i, 0.75f, 22.460001f);
                    d = _e116;
                    let _e118 = shadows_1;
                    let _e120 = d;
                    shadow = (0.7f * smoothstep(_e118, 0f, _e120));
                    let _e123 = saveD;
                    d = _e123;
                }
            }
            let _e124 = colorIn_1;
            tint_4 = _e124;
            let _e125 = insideLock_1;
            if (_e125 == 0i) {
                let _e128 = insideTransform_1;
                local_8 = _e128;
            } else {
                let _e129 = modelTransform_1;
                let _e130 = insideTransform_1;
                local_8 = (_e129 * _e130);
            }
            let _e133 = local_8;
            iTransform = _e133;
            let _e135 = iTransform;
            let _e137 = uv_1;
            let _e138 = tf(_naga_inverse_3x3_f32(_e135), _e137);
            v = _e138;
        }
    } else {
        {
            ...
            if (_e139 > 0f) {
                {
                    let _e142 = shadowTransform_1;
                    let _e144 = u_4;
                    let _e145 = tf(_naga_inverse_3x3_f32(_e142), _e144);
                    u_4 = _e145;
                    let _e146 = d;
                    saveD_1 = _e146;
                    let _e148 = d;
                    let _e149 = multiplier_1;
                    let _e151 = roundness_1;
                    d = ((_e148 * _e149) - _e151);
                    let _e153 = u_4;
                    let _e155 = u_4;
                    let _e168 = sdStar(vec2<f32>(_e153.x, -(_e155.y)), 24i, 0.75f, 22.460001f);
                    d = _e168;
                    let _e170 = shadows_1;
                    let _e172 = d;
                    shadow = (0.7f * smoothstep(_e170, 0f, _e172));
                    let _e175 = saveD_1;
                    d = _e175;
                }
            }
            let _e176 = colorOut_1;
            tint_4 = _e176;
        }
    }
    let _e177 = insideImage_specified_1;
    let _e180 = inside;
    if ((_e177 == 1i) && _e180) {
        let _e182 = v;
        let _e186 = global.U[0];
        let _e189 = v;
        let _e198 = _mirror_wrap(((vec2<f32>((_e182.x / _e186.x), _e189.y) / vec2(2f)) + vec2(0.5f)));
        let _e200 = textureSampleLevel(t_insideImage, samp, _e198, 0f);
        local_9 = _e200;
    } else {
        let _e201 = v;
        let _e205 = global.U[0];
        let _e208 = v;
        let _e217 = _mirror_wrap(((vec2<f32>((_e201.x / _e205.x), _e208.y) / vec2(2f)) + vec2(0.5f)));
        let _e219 = textureSampleLevel(t_source, samp, _e217, 0f);
        local_9 = _e219;
    }
    let _e221 = local_9;
    sColor = _e221;
    ...
    if _e225 {
        let _e226 = color;
        let _e227 = brightness_3;
        let _e228 = contrast_3;
        let _e231 = saturation_3;
        let _e232 = hue_3;
        let _e235 = adjustColorHSLuv(_e226, _e227, _e228, 0f, 0f, _e231, _e232, vec4(0f));
        color = _e235;
    }
    let _e236 = colorGlow_1;
    if (_e236.w != 0f) {
        let _e240 = colorGlow_1;
        let _e244 = d;
        let _e247 = ((_e240.xyz * 0.01f) / vec3(abs(_e244)));
        let _e249 = colorGlow_1;
        let _e253 = d;
        local_10 = vec4<f32>(_e247.x, _e247.y, _e247.z, min(1f, ((_e249.w * 0.01f) / abs(_e253))));
    } else {
        local_10 = vec4(0f);
    }
    let _e264 = local_10;
    ...
    let _e266 = color;
    let _e267 = tint_4;
    let _e268 = mergeColor(_e266, _e267);
    let _e269 = colorShadow_1;
    let _e270 = _e269.xyz;
    let _e271 = colorShadow_1;
    let _e273 = shadow;
    let _e279 = mergeColor(_e268, vec4<f32>(_e270.x, _e270.y, _e270.z, (_e271.w * _e273)));
    let _e280 = glow_2;
    let _e281 = mergeGlow(_e279, _e280);
    color = _e281;
    let _e282 = d;
    let _e284 = outlineThickness_1;
    if (abs(_e282) < (_e284 * 0.5f)) {
        {
            let _e288 = color;
            let _e289 = colorOutline_1;
            let _e290 = mergeColor(_e288, _e289);
            color = _e290;
            shadow = 0f;
        }
    }
