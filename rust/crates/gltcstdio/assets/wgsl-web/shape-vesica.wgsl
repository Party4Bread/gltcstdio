...
fn basicInterpolatedShapes(uv: vec2<f32>, outPos: vec2<f32>, insideImage_specified: i32, thickness: f32, shadows: f32, roundness: f32, multiplier: f32, colorOutline: vec4<f32>, outlineThickness: f32, brightness_2: f32, contrast_2: f32, saturation_2: f32, hue_2: f32, colorIn: vec4<f32>, colorOut: vec4<f32>, colorShadow: vec4<f32>, colorGlow: vec4<f32>, insideLock: i32, modelTransform: mat3x3<f32>, insideTransform: mat3x3<f32>, shadowTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    if ((_e152 == 1i) && _e155) {
        ...
        let _e175 = textureSampleLevel(t_insideImage, samp, _e173, 0f);
        local_10 = _e175;
    } else {
        let _e176 = v;
        let _e180 = global.U[0];
        let _e183 = v;
        let _e192 = _mirror_wrap(((vec2<f32>((_e176.x / _e180.x), _e183.y) / vec2(2f)) + vec2(0.5f)));
        let _e194 = textureSampleLevel(t_source, samp, _e192, 0f);
        local_10 = _e194;
    }
    let _e196 = local_10;
    sColor = _e196;
    ...
    if _e200 {
        let _e201 = color;
        let _e202 = brightness_3;
        let _e203 = contrast_3;
        let _e206 = saturation_3;
        let _e207 = hue_3;
        let _e210 = adjustColorHSLuv(_e201, _e202, _e203, 0f, 0f, _e206, _e207, vec4(0f));
        color = _e210;
    }
    let _e211 = colorGlow_1;
    if (_e211.w != 0f) {
        let _e215 = colorGlow_1;
        let _e219 = d_2;
        let _e222 = ((_e215.xyz * 0.01f) / vec3(abs(_e219)));
        let _e224 = colorGlow_1;
        let _e228 = d_2;
        local_11 = vec4<f32>(_e222.x, _e222.y, _e222.z, min(1f, ((_e224.w * 0.01f) / abs(_e228))));
    } else {
        local_11 = vec4(0f);
    }
    let _e239 = local_11;
    ...
    let _e241 = color;
    let _e242 = tint_4;
    let _e243 = mergeColor(_e241, _e242);
    let _e244 = colorShadow_1;
    let _e245 = _e244.xyz;
    let _e246 = colorShadow_1;
    let _e248 = shadow;
    let _e254 = mergeColor(_e243, vec4<f32>(_e245.x, _e245.y, _e245.z, (_e246.w * _e248)));
    let _e255 = glow_2;
    let _e256 = mergeGlow(_e254, _e255);
    color = _e256;
    let _e257 = d_2;
    let _e259 = outlineThickness_1;
    if (abs(_e257) < (_e259 * 0.5f)) {
        {
            let _e263 = color;
            let _e264 = colorOutline_1;
            let _e265 = mergeColor(_e263, _e264);
            color = _e265;
            shadow = 0f;
        }
    }
