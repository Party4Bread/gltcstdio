...
fn basicInterpolatedShapes(uv: vec2<f32>, outPos: vec2<f32>, insideImage_specified: i32, thickness: f32, shadows: f32, roundness: f32, multiplier: f32, colorOutline: vec4<f32>, outlineThickness: f32, brightness_2: f32, contrast_2: f32, saturation_2: f32, hue_2: f32, colorIn: vec4<f32>, colorOut: vec4<f32>, colorShadow: vec4<f32>, colorGlow: vec4<f32>, insideLock: i32, modelTransform: mat3x3<f32>, insideTransform: mat3x3<f32>, shadowTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    u_4 = _e52;
    let _e56 = u_4;
    let _e58 = sdDisk(_e56, 0.5f);
    let _e59 = u_4;
    let _e62 = thickness_1;
    let _e65 = sdDisk(_e59, (0.5f * (1f - _e62)));
    d = max(_e58, -(_e65));
    let _e68 = d;
    let _e69 = multiplier_1;
    let _e71 = roundness_1;
    d = ((_e68 * _e69) - _e71);
    let _e78 = uv_1;
    v = _e78;
    let _e80 = d;
    ...
    if _e84 {
        {
            ...
            if (_e85 < 0f) {
                {
                    ...
                    saveD = _e92;
                    let _e94 = d;
                    let _e95 = multiplier_1;
                    let _e97 = roundness_1;
                    d = ((_e94 * _e95) - _e97);
                    let _e99 = u_4;
                    let _e101 = sdDisk(_e99, 0.5f);
                    let _e102 = u_4;
                    let _e105 = thickness_1;
                    let _e108 = sdDisk(_e102, (0.5f * (1f - _e105)));
                    d = max(_e101, -(_e108));
                    let _e112 = shadows_1;
                    let _e114 = d;
                    shadow = (0.7f * smoothstep(_e112, 0f, _e114));
                    let _e117 = saveD;
                    d = _e117;
                }
            }
            let _e118 = colorIn_1;
            tint_4 = _e118;
            let _e119 = insideLock_1;
            if (_e119 == 0i) {
                let _e122 = insideTransform_1;
                local_8 = _e122;
            } else {
                let _e123 = modelTransform_1;
                let _e124 = insideTransform_1;
                local_8 = (_e123 * _e124);
            }
            let _e127 = local_8;
            iTransform = _e127;
            let _e129 = iTransform;
            let _e131 = uv_1;
    ...
    } else {
        {
            ...
            if (_e133 > 0f) {
                {
                    let _e136 = shadowTransform_1;
                    let _e138 = u_4;
                    let _e139 = tf(_naga_inverse_3x3_f32(_e136), _e138);
                    u_4 = _e139;
                    let _e140 = d;
                    saveD_1 = _e140;
                    let _e142 = d;
                    let _e143 = multiplier_1;
                    let _e145 = roundness_1;
                    d = ((_e142 * _e143) - _e145);
                    let _e147 = u_4;
                    let _e149 = sdDisk(_e147, 0.5f);
                    let _e150 = u_4;
                    let _e153 = thickness_1;
                    let _e156 = sdDisk(_e150, (0.5f * (1f - _e153)));
                    d = max(_e149, -(_e156));
                    let _e160 = shadows_1;
                    let _e162 = d;
                    shadow = (0.7f * smoothstep(_e160, 0f, _e162));
                    let _e165 = saveD_1;
                    d = _e165;
                }
            }
            let _e166 = colorOut_1;
            tint_4 = _e166;
        }
    }
    let _e167 = insideImage_specified_1;
    let _e170 = inside;
    if ((_e167 == 1i) && _e170) {
        let _e172 = v;
        ...
        let _e190 = textureSampleLevel(t_insideImage, samp, _e188, 0f);
        local_9 = _e190;
    } else {
        let _e191 = v;
        let _e195 = global.U[0];
        let _e198 = v;
        let _e207 = _mirror_wrap(((vec2<f32>((_e191.x / _e195.x), _e198.y) / vec2(2f)) + vec2(0.5f)));
        let _e209 = textureSampleLevel(t_source, samp, _e207, 0f);
        local_9 = _e209;
    }
    let _e211 = local_9;
    sColor = _e211;
    ...
    if _e215 {
        let _e216 = color;
        let _e217 = brightness_3;
        let _e218 = contrast_3;
        let _e221 = saturation_3;
        let _e222 = hue_3;
        let _e225 = adjustColorHSLuv(_e216, _e217, _e218, 0f, 0f, _e221, _e222, vec4(0f));
        color = _e225;
    }
    let _e226 = colorGlow_1;
    if (_e226.w != 0f) {
        let _e230 = colorGlow_1;
        let _e234 = d;
        let _e237 = ((_e230.xyz * 0.01f) / vec3(abs(_e234)));
        let _e239 = colorGlow_1;
        let _e243 = d;
        local_10 = vec4<f32>(_e237.x, _e237.y, _e237.z, min(1f, ((_e239.w * 0.01f) / abs(_e243))));
    } else {
        local_10 = vec4(0f);
    }
    let _e254 = local_10;
    ...
    let _e259 = colorShadow_1;
    let _e260 = _e259.xyz;
    let _e261 = colorShadow_1;
    let _e263 = shadow;
    let _e269 = mergeColor(_e258, vec4<f32>(_e260.x, _e260.y, _e260.z, (_e261.w * _e263)));
    let _e270 = glow_2;
    let _e271 = mergeGlow(_e269, _e270);
    color = _e271;
    let _e272 = d;
    let _e274 = outlineThickness_1;
    if (abs(_e272) < (_e274 * 0.5f)) {
        {
            let _e278 = color;
            let _e279 = colorOutline_1;
            let _e280 = mergeColor(_e278, _e279);
            color = _e280;
            shadow = 0f;
        }
    }
