...
fn displacedBorder(uv: vec2<f32>, outPos: vec2<f32>, border: f32, displacement_specified: i32, sourceDim: vec2<f32>, outDim: vec2<f32>, intensity: f32, balance: f32, colorOut: vec4<f32>, viewTransform: mat3x3<f32>, modelTransform: mat3x3<f32>, borderTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    if (_e72 == 1i) {
        ...
        let _e99 = textureSampleLevel(t_displacement, samp, _e97, 0f);
        local = _e99;
    } else {
        let _e100 = borderTransform_1;
        let _e102 = uv_1;
        let _e103 = tf(_naga_inverse_3x3_f32(_e100), _e102);
        let _e107 = global.U[0];
        let _e110 = borderTransform_1;
        let _e112 = uv_1;
        let _e113 = tf(_naga_inverse_3x3_f32(_e110), _e112);
        let _e122 = _mirror_wrap(((vec2<f32>((_e103.x / _e107.x), _e113.y) / vec2(2f)) + vec2(0.5f)));
        let _e124 = textureSampleLevel(t_source, samp, _e122, 0f);
        local = _e124;
    }
    let _e126 = local;
    let _e131 = balance_1;
    u_2 = (_e70 + (_e71 * ((_e126.xy - vec2(0.5f)) + vec2(_e131))));
    let _e136 = u_2;
    let _e139 = threshold;
    let _e142 = u_2;
    let _e145 = threshold;
    inside = ((abs(_e136.x) <= _e139.x) && (abs(_e142.y) <= _e145.y));
    let _e150 = modelTransform_1;
    let _e152 = uv_1;
    let _e153 = tf(_naga_inverse_3x3_f32(_e150), _e152);
    v = _e153;
    let _e155 = inside;
    if _e155 {
        let _e156 = v;
        ...
        let _e174 = textureSampleLevel(t_source, samp, _e172, 0f);
        local_1 = _e174;
    } else {
        let _e175 = v;
        ...
        let _e193 = textureSampleLevel(t_source, samp, _e191, 0f);
        let _e194 = colorOut_1;
        let _e195 = mergeColor(_e193, _e194);
        local_1 = _e195;
    }
    let _e197 = local_1;
    return _e197;
