...
fn basicInterpolatedShapes(uv: vec2<f32>, outPos: vec2<f32>, insideImage_specified: i32, shadows: f32, roundness: f32, multiplier: f32, colorOutline: vec4<f32>, outlineThickness: f32, brightness_2: f32, contrast_2: f32, saturation_2: f32, hue_2: f32, colorIn: vec4<f32>, colorOut: vec4<f32>, colorShadow: vec4<f32>, colorGlow: vec4<f32>, insideLock: i32, modelTransform: mat3x3<f32>, insideTransform: mat3x3<f32>, shadowTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    if ((_e156 == 1i) && _e159) {
        ...
        let _e179 = textureSampleLevel(t_insideImage, samp, _e177, 0f);
        local_9 = _e179;
    } else {
        let _e180 = v;
        ...
        let _e198 = textureSampleLevel(t_source, samp, _e196, 0f);
        local_9 = _e198;
    }
    let _e200 = local_9;
    sColor = _e200;
    ...
    if _e204 {
        let _e205 = color;
        let _e206 = brightness_3;
        let _e207 = contrast_3;
        let _e210 = saturation_3;
        let _e211 = hue_3;
        let _e214 = adjustColorHSLuv(_e205, _e206, _e207, 0f, 0f, _e210, _e211, vec4(0f));
        color = _e214;
    }
    let _e215 = colorGlow_1;
    if (_e215.w != 0f) {
        let _e219 = colorGlow_1;
        let _e223 = d;
        let _e226 = ((_e219.xyz * 0.01f) / vec3(abs(_e223)));
        let _e228 = colorGlow_1;
        let _e232 = d;
        local_10 = vec4<f32>(_e226.x, _e226.y, _e226.z, min(1f, ((_e228.w * 0.01f) / abs(_e232))));
    } else {
        local_10 = vec4(0f);
    }
    let _e243 = local_10;
    ...
    let _e245 = color;
    let _e246 = tint_4;
    let _e247 = mergeColor(_e245, _e246);
    let _e248 = colorShadow_1;
    let _e249 = _e248.xyz;
    let _e250 = colorShadow_1;
    let _e252 = shadow;
    let _e258 = mergeColor(_e247, vec4<f32>(_e249.x, _e249.y, _e249.z, (_e250.w * _e252)));
    let _e259 = glow_2;
    let _e260 = mergeGlow(_e258, _e259);
    color = _e260;
    let _e261 = d;
    let _e263 = outlineThickness_1;
    if (abs(_e261) < (_e263 * 0.5f)) {
        {
            let _e267 = color;
            let _e268 = colorOutline_1;
            let _e269 = mergeColor(_e267, _e268);
            color = _e269;
            shadow = 0f;
        }
    }
