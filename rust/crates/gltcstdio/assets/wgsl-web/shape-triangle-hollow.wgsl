...
fn basicInterpolatedShapes(uv: vec2<f32>, outPos: vec2<f32>, insideImage_specified: i32, thickness: f32, shadows: f32, roundness: f32, multiplier: f32, colorOutline: vec4<f32>, outlineThickness: f32, brightness_2: f32, contrast_2: f32, saturation_2: f32, hue_2: f32, colorIn: vec4<f32>, colorOut: vec4<f32>, colorShadow: vec4<f32>, colorGlow: vec4<f32>, insideLock: i32, modelTransform: mat3x3<f32>, insideTransform: mat3x3<f32>, shadowTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    let _e56 = u_4;
    let _e58 = u_4;
    let _e64 = sdEquiTriangle((vec2<f32>(_e56.x, -(_e58.y)) * 1.5f));
    let _e66 = u_4;
    let _e68 = u_4;
    let _e75 = thickness_1;
    let _e78 = sdEquiTriangle(((vec2<f32>(_e66.x, -(_e68.y)) * 1.5f) * (1f - _e75)));
    d = max(-(_e64), _e78);
    let _e80 = d;
    let _e81 = multiplier_1;
    let _e83 = roundness_1;
    d = ((_e80 * _e81) - _e83);
    let _e90 = uv_1;
    v = _e90;
    let _e92 = d;
    ...
    if _e96 {
        {
            ...
            if (_e97 < 0f) {
                {
                    let _e100 = shadowTransform_1;
                    let _e102 = u_4;
                    let _e103 = tf(_naga_inverse_3x3_f32(_e100), _e102);
                    u_4 = _e103;
                    let _e104 = d;
                    saveD = _e104;
                    let _e106 = d;
                    let _e107 = multiplier_1;
                    let _e109 = roundness_1;
                    d = ((_e106 * _e107) - _e109);
                    let _e111 = u_4;
                    let _e113 = u_4;
                    let _e119 = sdEquiTriangle((vec2<f32>(_e111.x, -(_e113.y)) * 1.5f));
                    let _e121 = u_4;
                    let _e123 = u_4;
                    let _e130 = thickness_1;
                    let _e133 = sdEquiTriangle(((vec2<f32>(_e121.x, -(_e123.y)) * 1.5f) * (1f - _e130)));
                    d = max(-(_e119), _e133);
                    let _e136 = shadows_1;
                    let _e138 = d;
                    shadow = (0.7f * smoothstep(_e136, 0f, _e138));
                    let _e141 = saveD;
                    d = _e141;
                }
            }
            let _e142 = colorIn_1;
            tint_4 = _e142;
            let _e143 = insideLock_1;
            if (_e143 == 0i) {
                let _e146 = insideTransform_1;
                local_8 = _e146;
            } else {
                let _e147 = modelTransform_1;
                let _e148 = insideTransform_1;
                local_8 = (_e147 * _e148);
            }
            let _e151 = local_8;
            iTransform = _e151;
            let _e153 = iTransform;
            let _e155 = uv_1;
            let _e156 = tf(_naga_inverse_3x3_f32(_e153), _e155);
            v = _e156;
        }
    } else {
        {
            ...
            if (_e157 > 0f) {
                {
                    let _e160 = shadowTransform_1;
                    let _e162 = u_4;
                    let _e163 = tf(_naga_inverse_3x3_f32(_e160), _e162);
                    u_4 = _e163;
                    let _e164 = d;
                    saveD_1 = _e164;
                    let _e166 = d;
                    let _e167 = multiplier_1;
                    let _e169 = roundness_1;
                    d = ((_e166 * _e167) - _e169);
                    let _e171 = u_4;
                    let _e173 = u_4;
                    let _e179 = sdEquiTriangle((vec2<f32>(_e171.x, -(_e173.y)) * 1.5f));
                    let _e181 = u_4;
                    let _e183 = u_4;
                    let _e190 = thickness_1;
                    let _e193 = sdEquiTriangle(((vec2<f32>(_e181.x, -(_e183.y)) * 1.5f) * (1f - _e190)));
                    d = max(-(_e179), _e193);
                    let _e196 = shadows_1;
                    let _e198 = d;
                    shadow = (0.7f * smoothstep(_e196, 0f, _e198));
                    let _e201 = saveD_1;
                    d = _e201;
                }
            }
            let _e202 = colorOut_1;
            tint_4 = _e202;
        }
    }
    let _e203 = insideImage_specified_1;
    let _e206 = inside;
    if ((_e203 == 1i) && _e206) {
        let _e208 = v;
        ...
        let _e226 = textureSampleLevel(t_insideImage, samp, _e224, 0f);
        local_9 = _e226;
    } else {
        let _e227 = v;
        let _e231 = global.U[0];
        let _e234 = v;
        let _e243 = _mirror_wrap(((vec2<f32>((_e227.x / _e231.x), _e234.y) / vec2(2f)) + vec2(0.5f)));
        let _e245 = textureSampleLevel(t_source, samp, _e243, 0f);
        local_9 = _e245;
    }
    let _e247 = local_9;
    sColor = _e247;
    ...
    if _e251 {
        let _e252 = color;
        let _e253 = brightness_3;
        let _e254 = contrast_3;
        let _e257 = saturation_3;
        let _e258 = hue_3;
        let _e261 = adjustColorHSLuv(_e252, _e253, _e254, 0f, 0f, _e257, _e258, vec4(0f));
        color = _e261;
    }
    let _e262 = colorGlow_1;
    if (_e262.w != 0f) {
        let _e266 = colorGlow_1;
        let _e270 = d;
        let _e273 = ((_e266.xyz * 0.01f) / vec3(abs(_e270)));
        let _e275 = colorGlow_1;
        let _e279 = d;
        local_10 = vec4<f32>(_e273.x, _e273.y, _e273.z, min(1f, ((_e275.w * 0.01f) / abs(_e279))));
    } else {
        local_10 = vec4(0f);
    }
    let _e290 = local_10;
    ...
    let _e292 = color;
    let _e293 = tint_4;
    let _e294 = mergeColor(_e292, _e293);
    let _e295 = colorShadow_1;
    let _e296 = _e295.xyz;
    let _e297 = colorShadow_1;
    let _e299 = shadow;
    let _e305 = mergeColor(_e294, vec4<f32>(_e296.x, _e296.y, _e296.z, (_e297.w * _e299)));
    let _e306 = glow_2;
    let _e307 = mergeGlow(_e305, _e306);
    color = _e307;
    let _e308 = d;
    let _e310 = outlineThickness_1;
    if (abs(_e308) < (_e310 * 0.5f)) {
        {
            let _e314 = color;
            let _e315 = colorOutline_1;
            let _e316 = mergeColor(_e314, _e315);
            color = _e316;
            shadow = 0f;
        }
    }
