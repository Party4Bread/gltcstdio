...
vec2 tf(mat3 m, vec2 u) {
    return (m * vec3(u, 1.)).xy;
        ...
        vec4 basicInterpolatedShapes(vec2 uv, vec2 outPos, int insideImage_specified, float shape, 
            ...
            float d = 0.0;
            if (shape<0.0) d = sdRectangle(u, vec2(0.5));
