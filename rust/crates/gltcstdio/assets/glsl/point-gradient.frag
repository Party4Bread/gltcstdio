...
#define u_colorIn (U[10])
#define u_colorOut (U[11])
...
void main() {
    fragColor = pointGradient((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_source_specified, u_mode, u_modelTransform, u_colorIn, u_colorOut);
}
