...
float sdf(vec3 p, float radius, float roundness) {
    return min(sdBox(p, vec3(.25)), length(abs(p)-vec3(.25))-radius*0.5) - roundness;
}

vec3 normal(vec3 p, float radius, float roundness) {
    float d = 0.0001;
    float s = sdf(p,radius,roundness);
    return normalize(vec3(
        (s-sdf(vec3(p.x-d, p.y, p.z),radius,roundness))/d,
        (s-sdf(vec3(p.x, p.y-d, p.z),radius,roundness))/d,
        (s-sdf(vec3(p.x, p.y, p.z-d),radius,roundness))/d
        ));
}

vec3 rayMarch(vec3 p0, vec3 dir, float side, float radius, float roundness) {
    float d = sdf(p0,radius,roundness);
    float s = sign(d);
    float totalD = 0.0;
    int step = 0;
    ...
    return vec3(INF);
        ...
        vec4 rayMarcher(vec2 uv, vec2 outPos, mat4 model3DTransform, vec2 sourceDim, mat4 lightSourceTransform, mat4 bkgTransform,
                        ...
                        vec4 sourceColor, vec4 ambientColor, float specular, int backgroundStyle, float radius, float roundness) {
            float D = 1.0;
...
//camDir = mat3(invModelTransform) * camDir; 
            ...
            vec3 qIn = rayMarch(camera, camDir, 1.,radius,roundness);
            ...
            if (qIn.x!=INF) {
                vec3 nIn = normal(qIn,radius,roundness);
                ...
                if (fresnel!=1.0) {
                    ...
                    vec3 qOut = rayMarch(qIn-nIn*0.001, refractDir, -1.,radius,roundness);

                    vec3 n = -normal(qOut,radius,roundness);
...
refractDirB = model3DTransform3 * refractDirB;
    ...
    colB = vec4((refractDirB)*0.5+0.5, 1.0);
                ...
                if (fresnel!=0.0 || specular!=0.0) {
                    ...
                    vec3 qR = rayMarch(qIn+nIn*0.001, reflectDir, 1.,radius,roundness);
                    if (qR.x!=INF) {
                         vec3 n = normal(qR,radius,roundness);
...
void main() {
    fragColor = rayMarcher((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_model3DTransform, u_sourceDim, u_lightSourceTransform, u_bkgTransform, u_camera3DTransform, u_colorMaterial, u_refractionIndex, u_fresnelStrength, u_chromaticAberration, u_colorFog, u_sourceColor, u_ambientColor, u_specular, u_backgroundStyle, u_radius, u_roundness);
}
