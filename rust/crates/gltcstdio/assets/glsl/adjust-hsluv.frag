...
#define u_hue (U[10].x)
#define u_tint (U[11])
#define u_vignette_intensity (U[12].x)
#define u_vignette_hardness (U[13].x)
#define u_vignette_color (U[14])
#define u_vignette_transform (mat3(U[15].xyz, U[16].xyz, U[17].xyz))
...
void main() {
    fragColor = adjustHSLuv((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_brightness, u_contrast, u_luminosity, u_gamma, u_saturation, u_hue, u_tint, u_vignette_intensity, u_vignette_hardness, u_vignette_color, u_vignette_transform);
}
