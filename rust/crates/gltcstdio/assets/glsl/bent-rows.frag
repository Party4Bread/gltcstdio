...
void main() {
    fragColor = bentRowsIllusion((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_source_specified, u_count, u_offset, u_thickness, u_color1, u_color2, u_color3);
}
