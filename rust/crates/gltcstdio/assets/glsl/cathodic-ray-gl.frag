...
vec4 cathodidRay(vec2 pos, vec2 outPos, float intensity, float balance, mat3 modelTransform) {
    ...
    return vec4(clamp(outCol.r, 0.0, 1.0), clamp(outCol.g, 0.0, 1.0),clamp(outCol.b, 0.0, 1.0), color.a);     
}

void main() {
    fragColor = cathodidRay((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_intensity, u_balance, u_modelTransform);
}
