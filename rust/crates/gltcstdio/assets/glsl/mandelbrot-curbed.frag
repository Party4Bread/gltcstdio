...
vec4 mandelbrotCurbed(vec2 pos, vec2 outPos, int source_specified, mat3 modelTransform, mat3 offsetTransform, mat3 texTransform, float intensity, int iterations, float julianess, float power) {
...
void main() {
    fragColor = mandelbrotCurbed((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_source_specified, u_modelTransform, u_offsetTransform, u_texTransform, u_intensity, u_iterations, u_julianess, u_power);
}
