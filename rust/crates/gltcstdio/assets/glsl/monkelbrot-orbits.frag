...
#define u_offsetTransform (mat3(U[10].xyz, U[11].xyz, U[12].xyz))
#define u_transformOrbit (mat3(U[13].xyz, U[14].xyz, U[15].xyz))
#define u_color (U[16])
#define u_offset (U[17].x)
#define u_colorPower (U[18].x)
#define u_iterations (int(U[19].x))
#define u_julianess (U[20].x)
#define u_power (U[21].x)
...
void threeOrbits(inout vec3 dist, vec2 z, mat3 t1, mat3 t2, mat3 t3, vec3 modes) {
        ...
        vec4 monkelbrotOrbits(vec2 pos, vec2 outPos, int source_specified, int mode, mat3 modelTransform, mat3 offsetTransform, mat3 transformOrbit, vec4 color, float offset, float colorPower, int iterations, float julianess, float power) {
...
vec3 modes = getOrbitModes(mode, transformOrbit, tR, tG, tB);
            ...
            if (power == 2.0) {
                while (iter < iterations) {
                    ++iter;
                    prev = z;
                    vec2 zz = complexMul(z, t) - vec2(1., 0.);
                    vec2 zzz = vec2(zz.x*zz.x - zz.y*zz.y - 1.0, 2.*zz.x*zz.y);
                    z.x = zzz.x*zzz.x - zzz.y*zzz.y - 1.0;
                    z.y = 2.*zzz.x*zzz.y;
                    
                    d2 = dot(z, z);
                    
                    threeOrbits(orbDist, z, tR, tG, tB, modes);
...
void main() {
    fragColor = monkelbrotOrbits((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_source_specified, u_mode, u_modelTransform, u_offsetTransform, u_transformOrbit, u_color, u_offset, u_colorPower, u_iterations, u_julianess, u_power);
}
