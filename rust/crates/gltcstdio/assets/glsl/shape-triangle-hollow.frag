...
vec2 tf(mat3 m, vec2 u) {
    return (m * vec3(u, 1.)).xy;
        ...
        vec4 basicInterpolatedShapes(vec2 uv, vec2 outPos, int insideImage_specified, float thickness, 
            ...
            float d = 0.0;
            //d = min(sdEquiTriangle(vec2(u.x, -u.y)*1.5), max(sdEquiTriangle(vec2(u.x, -u.y)*1.6), -sdEquiTriangle(vec2(u.x, -u.y)*1.7)));     
d = max(-sdEquiTriangle(vec2(u.x, -u.y)*1.5), sdEquiTriangle(vec2(u.x, -u.y)*1.5*(1.0-thickness)));     
