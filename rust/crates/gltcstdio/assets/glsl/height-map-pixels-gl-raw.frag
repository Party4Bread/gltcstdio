...
void main() {
    fragColor = heightMapPixelsGl((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_sourceBkg_specified, u_sourceElevation_specified, u_intensity, u_rezolution, u_thickness, u_sourceColor, u_ambientColor, u_colorScheme, u_specular, u_sourceDim, u_sourceElevationDim, u_model3DTransform);
}
