...
#define u_red (U[6].x)
#define u_green (U[7].x)
#define u_blue (U[8].x)
#define u_normalize (int(U[9].x))
...
void main() {
    fragColor = channelMixBW((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_intensity, u_red, u_green, u_blue, u_normalize);
}
