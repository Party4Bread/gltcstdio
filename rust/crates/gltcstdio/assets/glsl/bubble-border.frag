...
void main() {
    fragColor = bubbleBorder((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_border, u_sourceDim, u_outDim, u_borderColor, u_variability, u_randomSeed, u_modelTransform, u_borderTransform);
}
