...
void main() {
    fragColor = adjust((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_brightness, u_contrast, u_luminosity, u_gamma, u_saturation, u_hue, u_tint, u_vignette_intensity, u_vignette_hardness, u_vignette_color, u_vignette_transform);
}
