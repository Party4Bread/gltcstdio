...
#define u_transformOrbit (mat3(U[7].xyz, U[8].xyz, U[9].xyz))
#define u_color (U[10])
#define u_offset (U[11].x)
#define u_colorPower (U[12].x)
#define u_modelTransform (mat3(U[13].xyz, U[14].xyz, U[15].xyz))
#define u_iterations (int(U[16].x))
#define u_seed (U[17].x)
...
vec2 collatz(vec2 u, float a, float b, float c, float d, float e, float f, float g) {
    //return ((7.0*u+2.0) - mulI(5.0*u+2.0, expI(vec2(-PI*u.y, PI*u.x)))) * 0.25; // ORIGINAL
	return ((a*u+b) - complexMul(c*u+d, complexExp(vec2(e*u.y, f*u.x)))) * g; 
}
...
void threeOrbits(inout vec3 dist, vec2 z, mat3 t1, mat3 t2, mat3 t3, vec3 modes) {
        ...
        vec4 collatzOrbits(vec2 pos, vec2 outPos, int source_specified, int mode, mat3 transformOrbit, vec4 color, float offset, float colorPower, mat3 modelTransform, int iterations, float seed) {
...
vec3 modes = getOrbitModes(mode, transformOrbit, tR, tG, tB);
            ...
            for(int i=0; i<iterations; ++i) {//length(uv)<100.0) {
                uv = collatz(uv, pa, pb, pc, pd, pe ,pf, pg);  
...
void main() {
    fragColor = collatzOrbits((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_source_specified, u_mode, u_transformOrbit, u_color, u_offset, u_colorPower, u_modelTransform, u_iterations, u_seed);
}
