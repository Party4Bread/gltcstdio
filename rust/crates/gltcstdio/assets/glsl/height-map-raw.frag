...
#define u_lightSourceTransform (mat4(U[13], U[14], U[15], U[16]))
#define u_colorScheme (U[17].x)
#define u_sourceColor (U[18])
#define u_ambientColor (U[19])
#define u_colorFog (U[20])
#define u_normalSmoothing (U[21].x)
#define u_surfaceSmoothness (U[22].x)
#define u_specular (U[23].x)
#define u_shadows (U[24].x)
#define u_gamma (U[25].x)
...
#define __sourceElevation__(p) texture(u_sourceElevation, __mirror_wrap__(vec2((p).x / u_worldAspect, (p).y) / 2.0 + 0.5))
...
void main() {
    fragColor = heightMap((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_intensity, u_model3DTransform, u_sourceDim, u_sourceBkg_specified, u_sourceElevation_specified, u_lightSourceTransform, u_colorScheme, u_sourceColor, u_ambientColor, u_colorFog, u_normalSmoothing, u_surfaceSmoothness, u_specular, u_shadows, u_gamma);
}
