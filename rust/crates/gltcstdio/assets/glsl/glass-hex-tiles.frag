...
vec4 glassHexTiles(vec2 uv, vec2 outPos, float intensity, float distortion, float outAspectRatio, float pixelation, vec4 highFreqColor, mat3 modelTransform) {
...
void main() {
    fragColor = glassHexTiles((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_intensity, u_distortion, u_outAspectRatio, u_pixelation, u_highFreqColor, u_modelTransform);
}
