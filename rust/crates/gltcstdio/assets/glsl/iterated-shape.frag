...
#define u_saturation (U[13].x)
#define u_hue (U[14].x)
#define u_colorIn (U[15])
#define u_colorOut (U[16])
#define u_colorShadow (U[17])
#define u_colorGlow (U[18])
#define u_insideLock (int(U[19].x))
#define u_modelTransform (mat3(U[20].xyz, U[21].xyz, U[22].xyz))
#define u_insideTransform (mat3(U[23].xyz, U[24].xyz, U[25].xyz))
...
void main() {
    fragColor = iteratedShape((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_insideImage_specified, u_shape, u_iterations, u_shadows, u_roundness, u_multiplier, u_brightness, u_contrast, u_saturation, u_hue, u_colorIn, u_colorOut, u_colorShadow, u_colorGlow, u_insideLock, u_modelTransform, u_insideTransform);
}
