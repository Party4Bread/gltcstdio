...
float sdf(vec3 p) {
    ...
    vec3 qq = min(abs(q), 1.0) - 0.5;
    return length(qq) - 0.6;
}
...
vec3 normal(vec3 p) {
    ...
    return normalize(vec3(
        (s-sdf(vec3(p.x-d, p.y, p.z)))/d,
        (s-sdf(vec3(p.x, p.y-d, p.z)))/d,
        (s-sdf(vec3(p.x, p.y, p.z-d)))/d
        ));
}

vec3 rayMarch(vec3 p0, vec3 dir, float side) {
    float d = sdf(p0);
    float s = sign(d);
    float totalD = 0.0;
    int step = 0;
    ...
    return vec3(INF);
        ...
        vec4 rayMarcher(vec2 uv, vec2 outPos, mat4 model3DTransform, vec2 sourceDim, mat4 lightSourceTransform, mat4 bkgTransform,
                        ...
                        vec4 sourceColor, vec4 ambientColor, float specular, int backgroundStyle) {
            float D = 1.0;
...
//camDir = mat3(invModelTransform) * camDir; 
            ...
            vec3 qIn = rayMarch(camera, camDir, 1.);
            ...
            if (qIn.x!=INF) {
                ...
                if (fresnel!=1.0) {
                    ...
                    vec3 qOut = rayMarch(qIn-nIn*0.001, refractDir, -1.);
...
refractDirB = model3DTransform3 * refractDirB;
    ...
    colB = vec4((refractDirB)*0.5+0.5, 1.0);
                ...
                if (fresnel!=0.0 || specular!=0.0) {
                    ...
                    vec3 qR = rayMarch(qIn+nIn*0.001, reflectDir, 1.);
                    if (qR.x!=INF) {
                         vec3 n = normal(qR);
...
void main() {
    fragColor = rayMarcher((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_model3DTransform, u_sourceDim, u_lightSourceTransform, u_bkgTransform, u_camera3DTransform, u_colorMaterial, u_refractionIndex, u_fresnelStrength, u_chromaticAberration, u_colorFog, u_sourceColor, u_ambientColor, u_specular, u_backgroundStyle);
}
