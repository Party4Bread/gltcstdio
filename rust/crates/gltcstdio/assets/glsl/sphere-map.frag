...
#define u_countX (int(U[5].x))
#define u_countY (int(U[6].x))
...
vec4 sphereMap(vec2 pos, vec2 outPos, int countX, int countY, mat4 model3DTransform) {
...
void main() {
    fragColor = sphereMap((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_countX, u_countY, u_model3DTransform);
}
