...
#define u_sourceElevationDim (U[6].xy)
#define u_sourceBkg_specified (int(U[7].x))
#define u_sourceElevation_specified (int(U[8].x))
#define u_outDim (U[9].xy)
#define u_intensity (U[10].x)
#define u_rezolution (int(U[11].x))
#define u_thickness (U[12].x)
#define u_glow (U[13].x)
#define u_specular (U[14].x)
#define u_normalSmoothing (U[15].x)
#define u_model3DTransform (mat4(U[16], U[17], U[18], U[19]))
#define u_backgroundMode (int(U[20].x))
#define u_colorScheme (U[21].x)
#define u_reflectivity (U[22].x)
#define u_colorBkg (U[23])
#define u_colorLines (U[24])
#define u_colorFog (U[25])
#define u_colorSource (U[26])
#define u_colorAmbient (U[27])
...
void main() {
    fragColor = mesh3d((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_intensity, u_rezolution, u_thickness, u_glow, u_specular, u_normalSmoothing, u_model3DTransform, u_sourceDim, u_sourceBkgDim, u_sourceElevationDim, u_sourceBkg_specified, u_sourceElevation_specified, u_backgroundMode, u_colorScheme, u_reflectivity, u_colorBkg, u_colorLines, u_colorFog, u_colorSource, u_colorAmbient);
}
