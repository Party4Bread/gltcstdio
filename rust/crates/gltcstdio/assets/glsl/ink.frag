...
#define u_threshold (U[5].x)
#define u_hardness (U[6].x)
...
void main() {
    fragColor = duotone((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_threshold, u_hardness);
}
