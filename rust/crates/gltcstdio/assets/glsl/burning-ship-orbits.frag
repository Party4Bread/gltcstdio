...
void threeOrbits(inout vec3 dist, vec2 z, mat3 t1, mat3 t2, mat3 t3, vec3 modes) {
        ...
        vec4 burningShipOrbits(vec2 pos, vec2 outPos, int source_specified, int mode, mat3 modelTransform, mat3 offsetTransform, mat3 transformOrbit, vec4 color, float offset, float colorPower, int iterations, float julianess, float power) {
...
void main() {
    fragColor = burningShipOrbits((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_source_specified, u_mode, u_modelTransform, u_offsetTransform, u_transformOrbit, u_color, u_offset, u_colorPower, u_iterations, u_julianess, u_power);
}
