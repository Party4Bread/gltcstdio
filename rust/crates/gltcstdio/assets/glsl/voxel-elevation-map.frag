...
#define u_size (U[12].x)
#define u_sourceColor (U[13])
#define u_ambientColor (U[14])
#define u_colorFog (U[15])
...
void main() {
    fragColor = voxelElevationMap((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_sourceBkg_specified, u_sourceElevation_specified, u_sourceDim, u_sourceElevationDim, u_rezolution, u_intensity, u_specular, u_size, u_sourceColor, u_ambientColor, u_colorFog, u_model3DTransform);
}
