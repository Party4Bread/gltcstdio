...
void main() {
    fragColor = hyperbolicCirclePack((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_viewTransform, u_modelTransform, u_texTransform, u_size, u_thickness, u_variability);
}
