...
void main() {
    fragColor = inversion((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_mode, u_intensity, u_dampening, u_iterations, u_modelTransform, u_modelTransform2);
}
