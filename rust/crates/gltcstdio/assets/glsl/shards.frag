...
vec4 shards(vec2 pos, vec2 outPos, int mode, vec4 borderColor, float thickness, float regularity, float randomSeed, float balance, vec2 sourceDim, mat3 modelTransform) {
...
void main() {
    fragColor = shards((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_mode, u_borderColor, u_thickness, u_regularity, u_randomSeed, u_balance, u_sourceDim, u_modelTransform);
}
