...
#define u_axisTransform (mat3(U[9].xyz, U[10].xyz, U[11].xyz))
...
void main() {
    fragColor = mirrorOpt((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_sourceDim, u_modelTransform, u_axisTransform);
}
