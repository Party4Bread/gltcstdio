...
float sphereHitDist(vec3 center, float radius, vec3 origin, vec3 dir) {
    ...
    return -1.0;
        ...
        vec4 spheres(vec2 pos, vec2 outPos, mat4 model3DTransform, vec2 sourceDim, float intensity, float reflectivity, vec4 objectColor, vec4 glowColor, vec4 bkgColor, int backgroundStyle, int spheres_size) {
...
void main() {
    fragColor = spheres((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_model3DTransform, u_sourceDim, u_intensity, u_reflectivity, u_objectColor, u_glowColor, u_bkgColor, u_backgroundStyle, u_spheres_size);
}
