...
#define u_sampleTransform (mat3(U[11].xyz, U[12].xyz, U[13].xyz))
...
void main() {
    fragColor = hexTilesWithTextureTransformLegacy((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_intensity, u_distortion, u_pixelation, u_modelTransform, u_sampleTransform);
}
