...
#define u_scaleX (U[10].x)
#define u_scaleY (U[11].x)
...
void main() {
    fragColor = highFreqBanding((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_intensity, u_count, u_sourceDim, u_source2Dim, u_source2_specified, u_scaleX, u_scaleY, u_modelTransform);
}
