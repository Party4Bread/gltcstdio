...
#define u_multiplier (U[10].x)
#define u_colorOutline (U[11])
#define u_outlineThickness (U[12].x)
#define u_brightness (U[13].x)
#define u_contrast (U[14].x)
#define u_saturation (U[15].x)
#define u_hue (U[16].x)
#define u_colorIn (U[17])
#define u_colorOut (U[18])
#define u_colorShadow (U[19])
#define u_colorGlow (U[20])
#define u_insideLock (int(U[21].x))
#define u_modelTransform (mat3(U[22].xyz, U[23].xyz, U[24].xyz))
#define u_insideTransform (mat3(U[25].xyz, U[26].xyz, U[27].xyz))
#define u_shadowTransform (mat3(U[28].xyz, U[29].xyz, U[30].xyz))

#define __insideImage__texelFetch__(c) texelFetch(u_insideImage, (c), 0)
#define __insideImage__(p) texture(u_insideImage, __mirror_wrap__(vec2((p).x / u_worldAspect, (p).y) / 2.0 + 0.5))
...
vec4 basicInterpolatedShapes(vec2 uv, vec2 outPos, int insideImage_specified, int spikeCount, float radius, 
    ...
    float d = 0.0;
    d = sdStar(vec2(u.x, -u.y), spikeCount, 0.75, 2.0 + radius*(float(spikeCount)-2.0));
