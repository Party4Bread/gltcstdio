...
#define u_julianess (U[15].x)
#define u_power (U[16].x)
...
void main() {
    fragColor = mandelbrotFeather((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_source_specified, u_modelTransform, u_offsetTransform, u_iterations, u_dampening, u_balance, u_julianess, u_power, u_colorIn, u_colorOut);
}
