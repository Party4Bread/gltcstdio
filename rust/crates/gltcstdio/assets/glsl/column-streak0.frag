...
vec4 streak(vec2 uv, vec2 outPos, vec4 color, vec2 sourceDim, float thickness, float variability, float shadows, float randomSeed, float subdividing, mat3 modelTransform) {
...
void main() {
    fragColor = streak((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_color, u_sourceDim, u_thickness, u_variability, u_shadows, u_randomSeed, u_subdividing, u_modelTransform);
}
