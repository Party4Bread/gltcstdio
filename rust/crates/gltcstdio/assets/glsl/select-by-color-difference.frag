...
#define u_colorIn (U[5])
#define u_colorOut (U[6])
#define u_target (U[7].x)
...
void main() {
    fragColor = selectByColorDifference((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_colorIn, u_colorOut, u_target, u_tolerance, u_hardness);
}
