...
float torusImplicitFn(vec3 p, float radius, float perturbation) {
    float R = 0.5;
    float r = R * radius;
    if (perturbation != 0.0) {
        p += perturbation * 0.001 * vec3(
            sin(p.x * 10.1 + 0.4) * 0.5 * sin(p.y * 20.1 + 1.1),
            sin(p.y * 10.21 + 0.7) * 0.5 * sin(p.z * 20.22 + 0.1),
            sin(p.z * 10.021 + 0.8) * 0.5 * sin(p.x * 10.9 + 0.6)
        );
    }
...
vec3 torusNormal(vec3 p, float radius, float perturbation) {
    float d = 0.0001;
    float d2 = d * 2.0;
    return normalize(vec3(
        (torusImplicitFn(vec3(p.x - d, p.y, p.z), radius, perturbation) - torusImplicitFn(vec3(p.x + d, p.y, p.z), radius, perturbation)) / d2,
        (torusImplicitFn(vec3(p.x, p.y - d, p.z), radius, perturbation) - torusImplicitFn(vec3(p.x, p.y + d, p.z), radius, perturbation)) / d2,
        (torusImplicitFn(vec3(p.x, p.y, p.z - d), radius, perturbation) - torusImplicitFn(vec3(p.x, p.y, p.z + d), radius, perturbation)) / d2
    ));
}

vec2 torusBoundingSphereK(vec3 center, float radius, vec3 origin, vec3 dir) {
...
vec3 torusRayMarch(vec3 origin, vec3 dir, float radius, float perturbation, vec4 glowColor) {
    float minDist = 1e9;
    float k = 0.0;
    if (glowColor.r == 0.0 && glowColor.g == 0.0 && glowColor.b == 0.0 && perturbation == 0.0) {
        vec2 kBounds = torusBoundingSphereK(vec3(0.0), 0.5 * (1.0 + radius), origin, dir);
        float kk = kBounds.x;
    ...
    int iter = 0;
    vec3 p = origin;
    float dist = torusImplicitFn(p, radius, perturbation);
    float kStep = 1.0 - perturbation * 0.005;
    while (abs(dist) > de && iter < maxIter) {
        k += abs(dist) * kStep;
        p = origin + k * dir;
        dist = torusImplicitFn(p, radius, perturbation);
    ...
    return dist < de ? vec3(k, float(iter), minDist) : vec3(-1.0, float(iter), minDist);
        ...
        vec4 torusGl(vec2 pos, vec2 outPos, mat4 model3DTransform, vec2 sourceDim,
                     float intensity, float reflectivity, float radius, float perturbation,
            ...
            do {
                minK = 999999.99;
                minI = -1;
                vec3 inters = torusRayMarch(origin, dir, radius, perturbation, glowColor);
                float k = inters.x;
                ...
                if (minI >= 0) {
                    ...
                    vec3 normal = torusImplicitFn(origin, radius, perturbation) <= 0.0
                        ? torusNormal(intersection, radius, perturbation)
                        : -torusNormal(intersection, radius, perturbation);
                    if (iter == maxIter) {
...
void main() {
    fragColor = torusGl((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_model3DTransform, u_sourceDim, u_intensity, u_reflectivity, u_radius, u_perturbation, u_objectColor, u_glowColor, u_bkgColor, u_backgroundStyle);
}
