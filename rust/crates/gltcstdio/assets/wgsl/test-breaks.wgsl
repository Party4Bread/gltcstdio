...
fn testBreaks(uv: vec2<f32>, outPos: vec2<f32>, dampening: f32, perturbation: f32, distortion: f32, variability: f32, randomSeed: f32, modelTransform: mat3x3<f32>, displaceTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    let _e154 = dist;
    if ((_e151 > 0f) && (_e154 < 0f)) {
        {
            let _e158 = dist;
            let _e159 = dpdx(_e158);
            let _e160 = uv_1;
            let _e162 = dpdx(_e160.x);
            let _e164 = dist;
            let _e165 = dpdy(_e164);
            let _e166 = uv_1;
            let _e168 = dpdy(_e166.y);
            grad = normalize(vec2<f32>((_e159 / _e162), (_e165 / _e168)));
            let _e173 = dist;
            let _e175 = maxDist;
            r = (-(_e173) / _e175);
        ...
        if ((_e195 < 0f) && (_e198 > 0f)) {
            {
                let _e202 = dist;
                let _e203 = dpdx(_e202);
                let _e204 = uv_1;
                let _e206 = dpdx(_e204.x);
                let _e208 = dist;
                let _e209 = dpdy(_e208);
                let _e210 = uv_1;
                let _e212 = dpdy(_e210.y);
                grad_1 = normalize(vec2<f32>((_e203 / _e206), (_e209 / _e212)));
                let _e217 = dist;
                let _e218 = maxDist;
                r_1 = (_e217 / _e218);
                let _e221 = distortion_1;
                let _e226 = r_1;
                let _e230 = r_1;
                dp_1 = (((-(_e221) * 2f) * (1f - _e226)) / (0.5f + _e230));
                let _e234 = newPos;
                let _e235 = dp_1;
                let _e236 = grad_1;
                newPos = (_e234 + (_e235 * _e236));
            }
        }
    }
    let _e239 = newPos;
    ...
    let _e246 = newPos;
    let _e255 = _mirror_wrap(((vec2<f32>((_e239.x / _e243.x), _e246.y) / vec2(2f)) + vec2(0.5f)));
