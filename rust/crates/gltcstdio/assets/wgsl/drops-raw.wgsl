...
fn drops(uv: vec2<f32>, outPos: vec2<f32>, sourceBkg_specified: i32, intensity: f32, radius: f32, radiusVariability: f32, perturbation: f32, variability: f32, randomSeed: f32, modelTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    if (_e257 || (_e258 == 0i)) {
        ...
        let _e279 = textureSample(t_source, samp, _e278);
        local = _e279;
    } else {
        let _e280 = uv_1;
        let _e284 = global.U[0];
        let _e287 = uv_1;
        let _e296 = _mirror_wrap(((vec2<f32>((_e280.x / _e284.x), _e287.y) / vec2(2f)) + vec2(0.5f)));
        let _e297 = textureSample(t_sourceBkg, samp, _e296);
        local = _e297;
    }
    let _e299 = local;
    outColor = _e299;
