...
fn splitCombine(pos: vec2<f32>, outPos: vec2<f32>, dithering: f32, waviness: f32, axisTransform: mat3x3<f32>, viewTransform1_: mat3x3<f32>, viewTransform2_: mat3x3<f32>) -> vec4<f32> {
    ...
    if (_e61 < 0f) {
        ...
        let _e86 = textureSample(t_source1_, samp, ((vec2<f32>((_e67.x / _e71.x), _e77.y) / vec2(2f)) + vec2(0.5f)));
        local = _e86;
    } else {
        let _e87 = viewTransform2_1;
        let _e89 = pos_1;
        let _e90 = tf(_naga_inverse_3x3_f32(_e87), _e89);
        let _e94 = global.U[0];
        let _e97 = viewTransform2_1;
        let _e99 = pos_1;
        let _e100 = tf(_naga_inverse_3x3_f32(_e97), _e99);
        let _e109 = textureSample(t_source2_, samp, ((vec2<f32>((_e90.x / _e94.x), _e100.y) / vec2(2f)) + vec2(0.5f)));
        local = _e109;
    }
    let _e111 = local;
    color = _e111;
