...
fn duotone(pos: vec2<f32>, outPos: vec2<f32>, intensity: f32, color: vec4<f32>, normalization: f32, saturation: f32) -> vec4<f32> {
    var pos_1: vec2<f32>;
    ...
    col = _e34;
    let _e36 = col;
    let _e38 = luma(_e36.xyz);
    l = _e38;
    let _e40 = color_1;
    let _e42 = rgbToHsluv(_e40.xyz);
    hsluv = _e42;
    let _e45 = hsluv;
    let _e47 = saturation_1;
    hsluv.y = (_e45.y * _e47);
    let _e50 = hsluv;
    let _e54 = normalization_1;
    lim = mix(0.5f, (_e50.z * 0.01f), _e54);
    let _e57 = hsluv;
    let _e58 = hsluvToRgb(_e57);
    let _e59 = color_1;
    color2_ = vec4<f32>(_e58.x, _e58.y, _e58.z, _e59.w);
    let _e67 = l;
    let _e68 = lim;
    if (_e67 < _e68) {
        let _e75 = color2_;
        let _e76 = l;
        let _e77 = lim;
        tCol = mix(vec4<f32>(0f, 0f, 0f, 1f), _e75, vec4((_e76 / _e77)));
    } else {
        let _e81 = l;
        let _e82 = lim;
        if (_e81 > _e82) {
            ...
            let _e91 = lim;
            let _e94 = lim;
            tCol = mix(_e84, vec4<f32>(1f, 1f, 1f, 1f), vec4(((_e90 - _e91) / (1f - _e94))));
        } else {
            let _e99 = color2_;
            tCol = _e99;
        }
    ...
    return mix(_e100, _e101, vec4(_e102));
}
...
fn main_1() {
    ...
    let _e73 = global.U[7];
    let _e77 = global.U[8];
    let _e79 = duotone((_naga_inverse_3x3_f32(mat3x3<f32>(vec3<f32>(_e9.x, _e9.y, _e9.z), vec3<f32>(_e13.x, _e13.y, _e13.z), vec3<f32>(_e17.x, _e17.y, _e17.z))) * vec3<f32>(_e44.x, _e44.y, 1f)).xy, (((_e51 - vec2(0.5f)) * 2f) * vec2<f32>(_e59.x, 1f)), _e66.x, _e70, _e73.x, _e77.x);
    fragColor = _e79;
    return;
}
