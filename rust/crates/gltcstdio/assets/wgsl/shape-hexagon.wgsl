...
fn basicInterpolatedShapes(uv: vec2<f32>, outPos: vec2<f32>, insideImage_specified: i32, shadows: f32, roundness: f32, multiplier: f32, colorOutline: vec4<f32>, outlineThickness: f32, brightness_2: f32, contrast_2: f32, saturation_2: f32, hue_2: f32, colorIn: vec4<f32>, colorOut: vec4<f32>, colorShadow: vec4<f32>, colorGlow: vec4<f32>, insideLock: i32, modelTransform: mat3x3<f32>, insideTransform: mat3x3<f32>, shadowTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    if ((_e141 == 1i) && _e144) {
        ...
        let _e163 = textureSample(t_insideImage, samp, _e162);
        local_9 = _e163;
    } else {
        let _e164 = v;
    ...
    if _e187 {
        let _e188 = color;
        let _e189 = brightness_3;
        let _e190 = contrast_3;
        let _e193 = saturation_3;
        let _e194 = hue_3;
        let _e197 = adjustColorHSLuv(_e188, _e189, _e190, 0f, 0f, _e193, _e194, vec4(0f));
        color = _e197;
    }
    let _e198 = colorGlow_1;
    if (_e198.w != 0f) {
        let _e202 = colorGlow_1;
        let _e206 = d;
        let _e209 = ((_e202.xyz * 0.01f) / vec3(abs(_e206)));
        let _e211 = colorGlow_1;
        let _e215 = d;
        local_10 = vec4<f32>(_e209.x, _e209.y, _e209.z, min(1f, ((_e211.w * 0.01f) / abs(_e215))));
    } else {
        local_10 = vec4(0f);
    }
    let _e226 = local_10;
    ...
    let _e228 = color;
    let _e229 = tint_4;
    let _e230 = mergeColor(_e228, _e229);
    let _e231 = colorShadow_1;
    let _e232 = _e231.xyz;
    let _e233 = colorShadow_1;
    let _e235 = shadow;
    let _e241 = mergeColor(_e230, vec4<f32>(_e232.x, _e232.y, _e232.z, (_e233.w * _e235)));
    let _e242 = glow_2;
    let _e243 = mergeGlow(_e241, _e242);
    color = _e243;
    let _e244 = d;
    let _e246 = outlineThickness_1;
    if (abs(_e244) < (_e246 * 0.5f)) {
        {
            let _e250 = color;
            let _e251 = colorOutline_1;
