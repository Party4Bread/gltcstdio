...
fn streak(uv: vec2<f32>, outPos: vec2<f32>, color: vec4<f32>, sourceDim: vec2<f32>, thickness: f32, shadows: f32, randomness_variability: f32, randomness_seed: f32, subdividing: f32, modelTransform: mat3x3<f32>) -> vec4<f32> {
    var uv_1: vec2<f32>;
    ...
    var thickness_1: f32;
    var shadows_1: f32;
    var randomness_variability_1: f32;
    var randomness_seed_1: f32;
    ...
    shadows_1 = shadows;
    randomness_variability_1 = randomness_variability;
    randomness_seed_1 = randomness_seed;
    subdividing_1 = subdividing;
    modelTransform_1 = modelTransform;
    let _e26 = randomness_variability_1;
    variability = _e26;
    let _e28 = randomness_seed_1;
    randomSeed = _e28;
    let _e30 = modelTransform_1;
    let _e32 = uv_1;
    u = (_naga_inverse_3x3_f32(_e30) * vec3<f32>(_e32.x, _e32.y, 1f)).xy;
    let _e41 = sourceDim_1;
    pixel = (2f / _e41.y);
    let _e47 = modelTransform_1[0];
    scale = length(_e47.xy);
    let _e51 = thickness_1;
    let _e54 = scale;
    t = ((_e51 * 2f) * _e54);
    let _e57 = variability;
    var_ = (_e57 * 8f);
    let _e61 = u;
    index = floor((_e61.x + 0.5f));
    loop {
        let _e77 = k_4;
        if !((_e77 <= 6f)) {
            break;
    ...
    if (_e172 < 0f) {
        {
            let _e176 = subdividing_1;
            let _e177 = subdividing_1;
            let _e187 = var_;
            let _e189 = rnd;
            Y = (((50f / abs(((_e176 * _e177) * 10000f))) * 20f) * (1f + ((0.5f * _e187) * _e189.x)));
            let _e196 = subdividing_1;
            let _e197 = subdividing_1;
            let _e207 = var_;
            let _e209 = rnd;
        ...
        let _e276 = subdividing_1;
        if (_e276 > 0f) {
            {
                border = false;
                let _e280 = subdividing_1;
                let _e292 = var_;
                let _e294 = rnd;
                Y_1 = (((pow(abs((_e280 * 100f)), 1.5f) * 0.01f) * 20f) * (1f + ((0.01f * _e292) * _e294.x)));
                let _e301 = subdividing_1;
                let _e302 = subdividing_1;
                let _e312 = var_;
                let _e314 = rnd;
