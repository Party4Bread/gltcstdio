...
fn fisheye(uv: vec2<f32>, outPos: vec2<f32>, intensity: f32, highFreqColor: vec4<f32>) -> vec4<f32> {
    ...
    let _e30 = intensity_1;
    d = (tan((max(abs(_e20.x), abs(_e23.y)) * _e27)) / tan(_e30));
    let _e34 = d;
    let _e35 = a;
    let _e37 = a;
    uv_1 = (_e34 * vec2<f32>(cos(_e35), sin(_e37)));
    let _e43 = d;
    let _e45 = highFreqColor_1;
    kCol = smoothstep(0.5f, 5f, (abs(_e43) * _e45.w));
    let _e50 = uv_1;
    let _e54 = global.U[0];
    let _e57 = uv_1;
    let _e66 = textureSample(t_source, samp, ((vec2<f32>((_e50.x / _e54.x), _e57.y) / vec2(2f)) + vec2(0.5f)));
    let _e67 = highFreqColor_1;
    let _e68 = _e67.xyz;
    let _e74 = kCol;
    return mix(_e66, vec4<f32>(_e68.x, _e68.y, _e68.z, 1f), vec4(_e74));
}
