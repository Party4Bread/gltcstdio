...
fn basicInterpolatedShapes(uv: vec2<f32>, outPos: vec2<f32>, insideImage_specified: i32, thickness: f32, shadows: f32, roundness: f32, multiplier: f32, colorOutline: vec4<f32>, outlineThickness: f32, brightness_2: f32, contrast_2: f32, saturation_2: f32, hue_2: f32, colorIn: vec4<f32>, colorOut: vec4<f32>, colorShadow: vec4<f32>, colorGlow: vec4<f32>, insideLock: i32, modelTransform: mat3x3<f32>, insideTransform: mat3x3<f32>, shadowTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    if ((_e167 == 1i) && _e170) {
        ...
        let _e189 = textureSample(t_insideImage, samp, _e188);
        local_9 = _e189;
    } else {
        let _e190 = v;
        let _e194 = global.U[0];
        let _e197 = v;
        let _e206 = _mirror_wrap(((vec2<f32>((_e190.x / _e194.x), _e197.y) / vec2(2f)) + vec2(0.5f)));
        let _e207 = textureSample(t_source, samp, _e206);
        local_9 = _e207;
    }
    let _e209 = local_9;
    sColor = _e209;
    ...
    if _e213 {
        let _e214 = color;
        let _e215 = brightness_3;
        let _e216 = contrast_3;
        let _e219 = saturation_3;
        let _e220 = hue_3;
        let _e223 = adjustColorHSLuv(_e214, _e215, _e216, 0f, 0f, _e219, _e220, vec4(0f));
        color = _e223;
    }
    let _e224 = colorGlow_1;
    if (_e224.w != 0f) {
        let _e228 = colorGlow_1;
        let _e232 = d;
        let _e235 = ((_e228.xyz * 0.01f) / vec3(abs(_e232)));
        let _e237 = colorGlow_1;
        let _e241 = d;
        local_10 = vec4<f32>(_e235.x, _e235.y, _e235.z, min(1f, ((_e237.w * 0.01f) / abs(_e241))));
    } else {
        local_10 = vec4(0f);
    }
    let _e252 = local_10;
    ...
    let _e254 = color;
    let _e255 = tint_4;
    let _e256 = mergeColor(_e254, _e255);
    let _e257 = colorShadow_1;
    let _e258 = _e257.xyz;
    let _e259 = colorShadow_1;
    let _e261 = shadow;
    let _e267 = mergeColor(_e256, vec4<f32>(_e258.x, _e258.y, _e258.z, (_e259.w * _e261)));
    let _e268 = glow_2;
    let _e269 = mergeGlow(_e267, _e268);
    color = _e269;
    let _e270 = d;
    let _e272 = outlineThickness_1;
    if (abs(_e270) < (_e272 * 0.5f)) {
        {
            let _e276 = color;
            let _e277 = colorOutline_1;
            let _e278 = mergeColor(_e276, _e277);
            color = _e278;
            shadow = 0f;
        }
    }
