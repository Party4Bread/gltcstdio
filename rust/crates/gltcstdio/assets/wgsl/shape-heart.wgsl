...
fn basicInterpolatedShapes(uv: vec2<f32>, outPos: vec2<f32>, insideImage_specified: i32, shadows: f32, roundness: f32, multiplier: f32, colorOutline: vec4<f32>, outlineThickness: f32, brightness_2: f32, contrast_2: f32, saturation_2: f32, hue_2: f32, colorIn: vec4<f32>, colorOut: vec4<f32>, colorShadow: vec4<f32>, colorGlow: vec4<f32>, insideLock: i32, modelTransform: mat3x3<f32>, insideTransform: mat3x3<f32>, shadowTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    if ((_e153 == 1i) && _e156) {
        ...
        let _e175 = textureSample(t_insideImage, samp, _e174);
        local_9 = _e175;
    } else {
        let _e176 = v;
        ...
        let _e193 = textureSample(t_source, samp, _e192);
        local_9 = _e193;
    }
    let _e195 = local_9;
    sColor = _e195;
    ...
    if _e199 {
        let _e200 = color;
        let _e201 = brightness_3;
        let _e202 = contrast_3;
        let _e205 = saturation_3;
        let _e206 = hue_3;
        let _e209 = adjustColorHSLuv(_e200, _e201, _e202, 0f, 0f, _e205, _e206, vec4(0f));
        color = _e209;
    }
    let _e210 = colorGlow_1;
    if (_e210.w != 0f) {
        let _e214 = colorGlow_1;
        let _e218 = d;
        let _e221 = ((_e214.xyz * 0.01f) / vec3(abs(_e218)));
        let _e223 = colorGlow_1;
        let _e227 = d;
        local_10 = vec4<f32>(_e221.x, _e221.y, _e221.z, min(1f, ((_e223.w * 0.01f) / abs(_e227))));
    } else {
        local_10 = vec4(0f);
    }
    let _e238 = local_10;
    ...
    let _e240 = color;
    let _e241 = tint_4;
    let _e242 = mergeColor(_e240, _e241);
    let _e243 = colorShadow_1;
    let _e244 = _e243.xyz;
    let _e245 = colorShadow_1;
    let _e247 = shadow;
    let _e253 = mergeColor(_e242, vec4<f32>(_e244.x, _e244.y, _e244.z, (_e245.w * _e247)));
    let _e254 = glow_2;
    let _e255 = mergeGlow(_e253, _e254);
    color = _e255;
    let _e256 = d;
    let _e258 = outlineThickness_1;
    if (abs(_e256) < (_e258 * 0.5f)) {
        {
            let _e262 = color;
            let _e263 = colorOutline_1;
            let _e264 = mergeColor(_e262, _e263);
            color = _e264;
            shadow = 0f;
        }
    }
