...
fn hyperbolicSquare(uv: vec2<f32>, outPos: vec2<f32>, p_18: i32, q_3: i32, viewTransform: mat3x3<f32>, modelTransform: mat3x3<f32>, texTransform: mat3x3<f32>, aspectRatio: f32, offset_2: f32, boundary: i32, vignetting: f32) -> vec4<f32> {
    ...
    if _e542 {
        {
            ...
            let _e575 = textureSample(t_source, samp, _e574);
            col = _e575;
        }
    } else {
        {
            let _e576 = invTexTransform;
            let _e579 = tf(_e576, vec2(0f));
            v_2 = _e579;
            let _e581 = v_2;
            let _e585 = global.U[0];
            let _e588 = v_2;
            let _e597 = _mirror_wrap(((vec2<f32>((_e581.x / _e585.x), _e588.y) / vec2(2f)) + vec2(0.5f)));
            let _e598 = textureSample(t_source, samp, _e597);
            col = _e598;
        }
    }
    let _e602 = iterCount;
    capFade = (1f - smoothstep(80f, 100f, f32(_e602)));
    let _e607 = col;
    let _e609 = col;
    let _e611 = capFade;
    let _e612 = (_e609.xyz * _e611);
    col.x = _e612.x;
    col.y = _e612.y;
    col.z = _e612.z;
    let _e619 = vignetting_1;
    t_4 = _e619;
    let _e621 = t_4;
    s1_ = clamp((_e621 * 2f), 0f, 1f);
    let _e628 = t_4;
    s2_ = clamp(((_e628 - 0.5f) * 2f), 0f, 1f);
    let _e639 = s1_;
    cornerRadius = mix(0f, 0.1f, _e639);
    let _e643 = normUV;
    let _e646 = cornerRadius;
    let _e647 = sdRoundedBox(_e643, vec2(1f), _e646);
    distSquare = (1f + _e647);
    let _e652 = distSquare;
    darknessCorner = smoothstep(0.99f, 1f, _e652);
    let _e655 = normUV;
    let _e656 = normUV;
    a2_ = (_e655 * _e656);
    let _e659 = a2_;
    ...
    let _e663 = a4_;
    let _e665 = a4_;
    sd = sqrt(sqrt((_e663.x + _e665.y)));
    let _e673 = s2_;
    innerSd = mix(1.2f, 0.5f, _e673);
    let _e678 = s2_;
    outerSd = mix(1.4f, 1f, _e678);
    let _e681 = innerSd;
    let _e682 = outerSd;
    let _e683 = sd;
    darknessBody = smoothstep(_e681, _e682, _e683);
    let _e687 = darknessCorner;
    let _e690 = darknessBody;
    cornerMask = ((1f - _e687) * (1f - _e690));
    let _e695 = cornerMask;
    let _e696 = vigAmt;
    cornerMask = mix(1f, _e695, _e696);
    let _e698 = col;
    let _e700 = cornerMask;
    let _e701 = (_e698.xyz * _e700);
    let _e702 = col;
    return vec4<f32>(_e701.x, _e701.y, _e701.z, _e702.w);
}
