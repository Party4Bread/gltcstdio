...
fn basicInterpolatedShapes(uv: vec2<f32>, outPos: vec2<f32>, insideImage_specified: i32, thickness: f32, shadows: f32, roundness: f32, multiplier: f32, colorOutline: vec4<f32>, outlineThickness: f32, brightness_2: f32, contrast_2: f32, saturation_2: f32, hue_2: f32, colorIn: vec4<f32>, colorOut: vec4<f32>, colorShadow: vec4<f32>, colorGlow: vec4<f32>, insideLock: i32, modelTransform: mat3x3<f32>, insideTransform: mat3x3<f32>, shadowTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    if ((_e203 == 1i) && _e206) {
        ...
        let _e225 = textureSample(t_insideImage, samp, _e224);
        local_9 = _e225;
    } else {
        let _e226 = v;
        let _e230 = global.U[0];
        let _e233 = v;
        let _e242 = _mirror_wrap(((vec2<f32>((_e226.x / _e230.x), _e233.y) / vec2(2f)) + vec2(0.5f)));
        let _e243 = textureSample(t_source, samp, _e242);
        local_9 = _e243;
    }
    let _e245 = local_9;
    sColor = _e245;
    ...
    if _e249 {
        let _e250 = color;
        let _e251 = brightness_3;
        let _e252 = contrast_3;
        let _e255 = saturation_3;
        let _e256 = hue_3;
        let _e259 = adjustColorHSLuv(_e250, _e251, _e252, 0f, 0f, _e255, _e256, vec4(0f));
        color = _e259;
    }
    let _e260 = colorGlow_1;
    if (_e260.w != 0f) {
        let _e264 = colorGlow_1;
        let _e268 = d;
        let _e271 = ((_e264.xyz * 0.01f) / vec3(abs(_e268)));
        let _e273 = colorGlow_1;
        let _e277 = d;
        local_10 = vec4<f32>(_e271.x, _e271.y, _e271.z, min(1f, ((_e273.w * 0.01f) / abs(_e277))));
    } else {
        local_10 = vec4(0f);
    }
    let _e288 = local_10;
    ...
    let _e290 = color;
    let _e291 = tint_4;
    let _e292 = mergeColor(_e290, _e291);
    let _e293 = colorShadow_1;
    let _e294 = _e293.xyz;
    let _e295 = colorShadow_1;
    let _e297 = shadow;
    let _e303 = mergeColor(_e292, vec4<f32>(_e294.x, _e294.y, _e294.z, (_e295.w * _e297)));
    let _e304 = glow_2;
    let _e305 = mergeGlow(_e303, _e304);
    color = _e305;
    let _e306 = d;
    let _e308 = outlineThickness_1;
    if (abs(_e306) < (_e308 * 0.5f)) {
        {
            let _e312 = color;
            let _e313 = colorOutline_1;
            let _e314 = mergeColor(_e312, _e313);
            color = _e314;
            shadow = 0f;
        }
    }
