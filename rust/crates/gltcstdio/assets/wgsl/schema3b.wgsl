...
fn schema3b(uv: vec2<f32>, outPos: vec2<f32>, sourceDim: vec2<f32>, outDim: vec2<f32>, intensity_2: f32, iterations: i32, pixelation: f32, variability: f32, randomSeed: f32, color: vec4<f32>, thickness: f32, aspectRatio: f32, modelTransform: mat3x3<f32>, windowTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    loop {
        ...
        }
        {
            ...
            if (_e611 == 0i) {
                {
                    ...
                    if ((abs(dot((_e622 - _e623), _e625)) <= _e628) && (abs(dot((_e630 - _e631), _e633)) <= _e636)) {
                        ...
                        let _e656 = srcRatio;
                        let _e661 = srcRatio;
                        let _e673 = textureSample(t_source, samp, ((vec2<f32>((clamp(_e639, vec2<f32>(-(_e640), -1f), vec2<f32>(_e645, 1f)).x / _e652.x), clamp(_e655, vec2<f32>(-(_e656), -1f), vec2<f32>(_e661, 1f)).y) / vec2(2f)) + vec2(0.5f)));
                        return _e673;
                    }
                }
            }
            ...
            let _e675 = rect_2;
            let _e676 = splits_2;
            let _e677 = intensity_3;
            let _e678 = randomSeed_1;
            let _e679 = distort9_(_e674, _e675, _e676, _e677, _e678);
            p_2 = _e679;
        }
        continuing {
            let _e141 = j;
    ...
    let _e680 = p_2;
    ps = _e680;
    let _e682 = pixelation_1;
    if (_e682 > 0.0001f) {
        let _e685 = p_2;
        let _e686 = pixelation_1;
        let _e693 = pixelation_1;
        ps = (floor(((_e685 / vec2(_e686)) + vec2(0.5f))) * _e693);
    }
    let _e695 = border;
    if _e695 {
        {
            let _e696 = uv_1;
            let _e700 = global.U[0];
            let _e703 = uv_1;
            let _e712 = textureSample(t_source, samp, ((vec2<f32>((_e696.x / _e700.x), _e703.y) / vec2(2f)) + vec2(0.5f)));
            col = _e712;
            let _e714 = col;
            let _e716 = color_1;
            let _e718 = color_1;
            let _e721 = mix(_e714.xyz, _e716.xyz, vec3(_e718.w));
            let _e722 = col;
            return vec4<f32>(_e721.x, _e721.y, _e721.z, _e722.w);
        }
    }
    let _e728 = ps;
