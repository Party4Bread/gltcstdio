...
fn basicInterpolatedShapes(uv: vec2<f32>, outPos: vec2<f32>, insideImage_specified: i32, shadows: f32, roundness: f32, multiplier: f32, colorOutline: vec4<f32>, outlineThickness: f32, brightness_2: f32, contrast_2: f32, saturation_2: f32, hue_2: f32, colorIn: vec4<f32>, colorOut: vec4<f32>, colorShadow: vec4<f32>, colorGlow: vec4<f32>, insideLock: i32, modelTransform: mat3x3<f32>, insideTransform: mat3x3<f32>, shadowTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    let _e54 = u_4;
    let _e56 = u_4;
    let _e62 = sdEquiTriangle((vec2<f32>(_e54.x, -(_e56.y)) * 1.5f));
    d = _e62;
    let _e63 = d;
    let _e64 = multiplier_1;
    let _e66 = roundness_1;
    d = ((_e63 * _e64) - _e66);
    let _e73 = uv_1;
    v = _e73;
    let _e75 = d;
    ...
    if _e79 {
        {
            ...
            if (_e80 < 0f) {
                {
                    ...
                    saveD = _e87;
                    let _e89 = d;
                    let _e90 = multiplier_1;
                    let _e92 = roundness_1;
                    d = ((_e89 * _e90) - _e92);
                    let _e94 = u_4;
                    let _e96 = u_4;
                    let _e102 = sdEquiTriangle((vec2<f32>(_e94.x, -(_e96.y)) * 1.5f));
                    d = _e102;
                    let _e104 = shadows_1;
                    let _e106 = d;
                    shadow = (0.7f * smoothstep(_e104, 0f, _e106));
                    let _e109 = saveD;
                    d = _e109;
                }
            }
            let _e110 = colorIn_1;
            tint_4 = _e110;
            let _e111 = insideLock_1;
            if (_e111 == 0i) {
                let _e114 = insideTransform_1;
                local_8 = _e114;
            } else {
                let _e115 = modelTransform_1;
                let _e116 = insideTransform_1;
                local_8 = (_e115 * _e116);
            }
            let _e119 = local_8;
            iTransform = _e119;
            let _e121 = iTransform;
            let _e123 = uv_1;
            let _e124 = tf(_naga_inverse_3x3_f32(_e121), _e123);
            v = _e124;
        }
    } else {
        {
            ...
            if (_e125 > 0f) {
                {
                    ...
                    let _e139 = u_4;
                    let _e141 = u_4;
                    let _e147 = sdEquiTriangle((vec2<f32>(_e139.x, -(_e141.y)) * 1.5f));
                    d = _e147;
                    let _e149 = shadows_1;
                    let _e151 = d;
                    shadow = (0.7f * smoothstep(_e149, 0f, _e151));
                    let _e154 = saveD_1;
                    d = _e154;
                }
            }
            let _e155 = colorOut_1;
            tint_4 = _e155;
        }
    }
    let _e156 = insideImage_specified_1;
    let _e159 = inside;
    if ((_e156 == 1i) && _e159) {
        let _e161 = v;
        ...
        let _e178 = textureSample(t_insideImage, samp, _e177);
        local_9 = _e178;
    } else {
        let _e179 = v;
        let _e183 = global.U[0];
        let _e186 = v;
        let _e195 = _mirror_wrap(((vec2<f32>((_e179.x / _e183.x), _e186.y) / vec2(2f)) + vec2(0.5f)));
        let _e196 = textureSample(t_source, samp, _e195);
        local_9 = _e196;
    }
    let _e198 = local_9;
    sColor = _e198;
    ...
    if _e202 {
        let _e203 = color;
        let _e204 = brightness_3;
        let _e205 = contrast_3;
        let _e208 = saturation_3;
        let _e209 = hue_3;
        let _e212 = adjustColorHSLuv(_e203, _e204, _e205, 0f, 0f, _e208, _e209, vec4(0f));
        color = _e212;
    }
    let _e213 = colorGlow_1;
    if (_e213.w != 0f) {
        let _e217 = colorGlow_1;
        let _e221 = d;
        let _e224 = ((_e217.xyz * 0.01f) / vec3(abs(_e221)));
        let _e226 = colorGlow_1;
        let _e230 = d;
        local_10 = vec4<f32>(_e224.x, _e224.y, _e224.z, min(1f, ((_e226.w * 0.01f) / abs(_e230))));
    } else {
        local_10 = vec4(0f);
    }
    let _e241 = local_10;
    ...
    let _e247 = _e246.xyz;
    let _e248 = colorShadow_1;
    let _e250 = shadow;
    let _e256 = mergeColor(_e245, vec4<f32>(_e247.x, _e247.y, _e247.z, (_e248.w * _e250)));
    let _e257 = glow_2;
    let _e258 = mergeGlow(_e256, _e257);
    color = _e258;
    let _e259 = d;
    let _e261 = outlineThickness_1;
    if (abs(_e259) < (_e261 * 0.5f)) {
        {
            let _e265 = color;
            let _e266 = colorOutline_1;
            let _e267 = mergeColor(_e265, _e266);
            color = _e267;
            shadow = 0f;
        }
    }
