...
fn shredCombine(pos: vec2<f32>, outPos: vec2<f32>, shadows: f32, intensity_2: f32, thickness: f32, borderColor: vec4<f32>, colorShadow: vec4<f32>, axisTransform: mat3x3<f32>, viewTransform1_: mat3x3<f32>, viewTransform2_: mat3x3<f32>) -> vec4<f32> {
    ...
    if (_e62 < 0f) {
        ...
        let _e87 = textureSample(t_source1_, samp, ((vec2<f32>((_e68.x / _e72.x), _e78.y) / vec2(2f)) + vec2(0.5f)));
        outCol = _e87;
    } else {
        ...
        let _e110 = textureSample(t_source2_, samp, ((vec2<f32>((_e91.x / _e95.x), _e101.y) / vec2(2f)) + vec2(0.5f)));
        outCol = _e110;
    }
    let _e111 = shadows_1;
    if (_e111 != 0f) {
        {
            ...
            let _e116 = d;
            let _e118 = th;
            dShadow = ((sign(_e114) * _e116) - _e118);
            let _e121 = dShadow;
            if (_e121 > 0f) {
                {
                    let _e124 = shadows_1;
                    let _e126 = shadows_1;
                    let _e130 = dShadow;
                    shadowStrength = smoothstep(abs(_e124), (abs(_e126) * 0.25f), _e130);
                    let _e133 = colorShadow_1;
                    let _e134 = _e133.xyz;
                    let _e135 = colorShadow_1;
                    let _e137 = shadowStrength;
                    shColor = vec4<f32>(_e134.x, _e134.y, _e134.z, (_e135.w * _e137));
                    let _e144 = outCol;
                    let _e145 = shColor;
                    let _e146 = mergeColor(_e144, _e145);
                    outCol = _e146;
                }
            }
        }
