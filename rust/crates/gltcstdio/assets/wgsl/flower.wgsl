...
fn flower(uv: vec2<f32>, outPos: vec2<f32>, spikeCount: i32, intensity: f32, dampening: f32, shape: f32, variability: f32, randomSeed: f32, lighting: f32, modelTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    if (_e37 >= 1f) {
    ...
    } else {
        {
            ...
            outCol = _e226;
            let _e228 = lighting_1;
            if (_e228 > 0f) {
                {
                    let _e231 = coord;
                    let _e232 = u_2;
                    dilation = length((_e231 - _e232));
                    let _e236 = dilation;
                    let _e237 = dpdx(_e236);
                    let _e238 = u_2;
                    let _e240 = dpdx(_e238.x);
                    let _e242 = dilation;
                    let _e243 = dpdy(_e242);
                    let _e244 = u_2;
                    let _e246 = dpdy(_e244.y);
                    grad = (vec2<f32>((_e237 / _e240), (_e243 / _e246)) * 4f);
