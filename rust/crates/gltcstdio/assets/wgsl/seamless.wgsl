    ...
    if ((_e169.x != 1f) || (_e173.y != 1f)) {
        {
            ...
            let _e189 = u2_;
            let _e191 = u2_;
            let _e202 = textureSample(t_source, samp, ((vec2<f32>((vec2<f32>(_e178.x, _e180.y).x / _e186.x), vec2<f32>(_e189.x, _e191.y).y) / vec2(2f)) + vec2(0.5f)));
            let _e203 = u;
            let _e205 = u2_;
            let _e211 = global.U[0];
            let _e214 = u;
            let _e216 = u2_;
            let _e227 = textureSample(t_source, samp, ((vec2<f32>((vec2<f32>(_e203.x, _e205.y).x / _e211.x), vec2<f32>(_e214.x, _e216.y).y) / vec2(2f)) + vec2(0.5f)));
            let _e228 = k;
            let _e232 = u2_;
            let _e234 = u;
            ...
            let _e243 = u2_;
            let _e245 = u;
            let _e256 = textureSample(t_source, samp, ((vec2<f32>((vec2<f32>(_e232.x, _e234.y).x / _e240.x), vec2<f32>(_e243.x, _e245.y).y) / vec2(2f)) + vec2(0.5f)));
            let _e257 = u;
            let _e261 = global.U[0];
            let _e264 = u;
            let _e273 = textureSample(t_source, samp, ((vec2<f32>((_e257.x / _e261.x), _e264.y) / vec2(2f)) + vec2(0.5f)));
            let _e274 = k;
            let _e278 = k;
            return mix(mix(_e202, _e227, vec4(_e228.x)), mix(_e256, _e273, vec4(_e274.x)), vec4(_e278.y));
        }
    } else {
        {
            let _e282 = u;
            ...
            let _e289 = u;
            let _e298 = textureSample(t_source, samp, ((vec2<f32>((_e282.x / _e286.x), _e289.y) / vec2(2f)) + vec2(0.5f)));
            return _e298;
        }
    }
}
