    ...
    if (_e28 >= 1f) {
        {
            ...
            let _e48 = textureSample(t_source, samp, _e47);
            return _e48;
        }
    } else {
        {
            ...
            let _e85 = d;
            dangle = ((_e81 * 10f) * (1f - cos(((_e85 * 2f) * 3.1415927f))));
            let _e94 = dangle;
            ca = cos(_e94);
            let _e97 = dangle;
            sa = sin(_e97);
            ...
            let _e113 = sa;
            let _e114 = u_2;
            rotated = vec2<f32>(((_e100 * _e101.x) - (_e104 * _e105.y)), ((_e109 * _e110.y) + (_e113 * _e114.x)));
            let _e122 = highFreqColor_1;
            if (_e122.w != 0f) {
                {
                    let _e126 = rotated;
                    let _e129 = intensity_1;
                    d_1 = length((_e126 * vec2<f32>(min(1.5f, (1f + abs((_e129 * 3f)))), 1f)));
                    let _e140 = highFreqColor_1;
                    sHeight = (_e140.w * 4f);
                    let _e146 = highFreqColor_1;
                    sSlope = (1f + (_e146.w * 3f));
                    let _e152 = sHeight;
                    let _e153 = d_1;
                    let _e154 = sSlope;
                    darken = clamp((_e152 - (_e153 * _e154)), 0f, 1f);
                }
            }
            let _e160 = modelTransform_1;
            let _e161 = rotated;
            let _e162 = tf(_e160, _e161);
            coord = _e162;
            let _e164 = coord;
