...
fn colorize(sourceColor: vec4<f32>, targetColor: vec4<f32>, saturation: f32) -> vec4<f32> {
    ...
    let _e12 = targetColor_1;
    let _e13 = rgbToHsl(_e12);
    hslTarget = _e13;
    let _e15 = sourceColor_1;
    ...
    if (_e22.y == 0f) {
        ...
        let _e27 = hslTarget;
        let _e29 = saturation_1;
        let _e31 = hslSource;
        let _e34 = saturation_1;
        local_2 = ((_e27.y * _e29) + (_e31.y * (1f - _e34)));
    }
    let _e39 = local_2;
    hslSource.y = _e39;
    let _e42 = hslTarget;
    gamma = pow(2f, ((0.5f - _e42.z) * 2f));
    let _e50 = hslSource;
    let _e52 = gamma;
    hslSource.z = pow(_e50.z, _e52);
    let _e54 = hslSource;
    let _e55 = hslToRgb(_e54);
    return _e55;
}

fn emphasizeColor(pos: vec2<f32>, outPos: vec2<f32>, color: vec4<f32>, intensity: f32, saturation_2: f32, tolerance: f32, hardness: f32) -> vec4<f32> {
    var pos_1: vec2<f32>;
    ...
    let _e51 = inHsl;
    let _e53 = empHsl;
    delta = vec2<f32>(((_e44.x - _e46.x) / 180f), (_e51.y - _e53.y));
    let _e58 = delta;
    if (_e58.x > 1f) {
        let _e64 = delta;
        delta.x = (2f - _e64.x);
    ...
    let _e67 = delta;
    dist = (length(_e67) / 1.4f);
    let _e72 = dist;
    let _e73 = tolerance_1;
    if (_e72 >= _e73) {
        let _e75 = inc_2;
        return _e75;
    }
    let _e76 = inc_2;
    let _e77 = color_1;
    let _e78 = saturation_3;
    let _e79 = colorize(_e76, _e77, _e78);
    rgb = _e79;
    let _e81 = intensity_1;
    let _e82 = tolerance_1;
    let _e85 = tolerance_1;
    let _e86 = hardness_1;
    let _e88 = dist;
    intens = (_e81 * smoothstep((_e82 * 1.001f), (_e85 * _e86), _e88));
    let _e92 = inc_2;
    let _e93 = rgb;
    let _e94 = intens;
    return mix(_e92, _e93, vec4(_e94));
}
...
fn main_1() {
    ...
    let _e77 = global.U[8];
    let _e81 = global.U[9];
    let _e83 = emphasizeColor((_naga_inverse_3x3_f32(mat3x3<f32>(vec3<f32>(_e9.x, _e9.y, _e9.z), vec3<f32>(_e13.x, _e13.y, _e13.z), vec3<f32>(_e17.x, _e17.y, _e17.z))) * vec3<f32>(_e44.x, _e44.y, 1f)).xy, (((_e51 - vec2(0.5f)) * 2f) * vec2<f32>(_e59.x, 1f)), _e66, _e69.x, _e73.x, _e77.x, _e81.x);
    fragColor = _e83;
    return;
}
