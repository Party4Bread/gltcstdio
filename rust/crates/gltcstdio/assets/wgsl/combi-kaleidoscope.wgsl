...
fn combiKaleidoscope(uv: vec2<f32>, outPos: vec2<f32>, source2_specified: i32, spikeCount_4: i32, border: f32, shadows: f32, borderColor: vec4<f32>, colorShadow: vec4<f32>, blend: f32, transformPairing: i32, shape_2: f32, modelTransform: mat3x3<f32>, modelTransform2_: mat3x3<f32>, borderTransform: mat3x3<f32>, offset: f32, stretch: f32) -> vec4<f32> {
    ...
    if (_e177 == 0f) {
        {
            ...
            if _e180 {
                ...
                let _e198 = textureSample(t_source1_, samp, _e197);
                local_3 = _e198;
            } else {
                let _e199 = source2_specified_1;
                if (_e199 != 1i) {
                    let _e202 = u2_;
                    let _e206 = global.U[0];
                    let _e209 = u2_;
                    let _e218 = _mirror_wrap(((vec2<f32>((_e202.x / _e206.x), _e209.y) / vec2(2f)) + vec2(0.5f)));
                    let _e219 = textureSample(t_source1_, samp, _e218);
                    local_2 = _e219;
                } else {
                    let _e220 = u2_;
                    let _e224 = global.U[0];
                    let _e227 = u2_;
                    let _e236 = _mirror_wrap(((vec2<f32>((_e220.x / _e224.x), _e227.y) / vec2(2f)) + vec2(0.5f)));
                    let _e237 = textureSample(t_source2_, samp, _e236);
                    local_2 = _e237;
                }
                let _e239 = local_2;
                local_3 = _e239;
    ...
    } else {
        {
            ...
            let _e249 = u1_;
            let _e258 = _mirror_wrap(((vec2<f32>((_e242.x / _e246.x), _e249.y) / vec2(2f)) + vec2(0.5f)));
            let _e259 = textureSample(t_source1_, samp, _e258);
            c1_ = _e259;
            let _e261 = source2_specified_1;
            if (_e261 != 1i) {
                let _e264 = u2_;
                let _e268 = global.U[0];
                let _e271 = u2_;
                let _e280 = _mirror_wrap(((vec2<f32>((_e264.x / _e268.x), _e271.y) / vec2(2f)) + vec2(0.5f)));
                let _e281 = textureSample(t_source1_, samp, _e280);
                local_4 = _e281;
            } else {
                let _e282 = u2_;
                let _e286 = global.U[0];
                let _e289 = u2_;
                let _e298 = _mirror_wrap(((vec2<f32>((_e282.x / _e286.x), _e289.y) / vec2(2f)) + vec2(0.5f)));
                let _e299 = textureSample(t_source2_, samp, _e298);
                local_4 = _e299;
            }
            let _e301 = local_4;
            c2_ = _e301;
            ...
            let _e305 = blend_1;
            let _e306 = dist;
            bk = smoothstep(-(_e303), _e305, _e306);
            let _e309 = c1_;
            let _e310 = c2_;
            let _e311 = bk;
    ...
    if (_e317 < (_e318 * 0.1f)) {
        let _e322 = outColor;
        let _e323 = borderColor_1;
        let _e324 = mergeColor(_e322, _e323);
        outColor = _e324;
    } else {
        let _e325 = adist;
        let _e326 = shadows_1;
        if (_e325 < abs(_e326)) {
            {
                ...
                let _e344 = dist;
                let _e345 = shadows_1;
                if ((((_e329 < 0f) && _e332) && (_e334 > _e335)) || (((_e338 > 0f) && !(_e341)) && (_e344 < _e345))) {
                    let _e349 = dist;
                    let _e351 = shadows_1;
                    local_5 = (abs(_e349) / abs(_e351));
                } else {
                    local_5 = 1f;
                }
                let _e356 = local_5;
                ds = _e356;
                let _e358 = outColor;
                let _e359 = colorShadow_1;
                let _e360 = _e359.xyz;
                let _e361 = colorShadow_1;
                let _e367 = ds;
                let _e374 = mergeColor(_e358, vec4<f32>(_e360.x, _e360.y, _e360.z, ((_e361.w * 0.8f) * smoothstep(1f, 0f, _e367))));
                outColor = _e374;
            }
        }
    }
