    ...
    if (abs(_e156) <= _e158) {
        {
            ...
            let _e184 = textureSample(t_source, samp, ((vec2<f32>((_e166.x / _e170.x), _e175.y) / vec2(2f)) + vec2(0.5f)));
            return _e184;
        }
    } else {
        {
            let _e185 = xpp;
            let _e187 = blendStart;
            let _e189 = blend_1;
            k = ((abs(_e185) - _e187) / _e189);
            let _e192 = sx;
            let _e193 = sy;
            pos1_ = vec2<f32>(_e192, _e193);
            let _e196 = xp;
            if (_e196 >= 0f) {
                let _e199 = sx;
                let _e200 = blendedRatio;
                local_2 = (_e199 - (_e200 * 2f));
            } else {
                let _e204 = sx;
                let _e205 = blendedRatio;
                local_2 = (_e204 + (_e205 * 2f));
            }
            let _e210 = local_2;
            sx2_ = _e210;
            let _e212 = sx2_;
            let _e213 = sy;
            pos2_ = vec2<f32>(_e212, _e213);
            let _e216 = inverseTexTransform;
            let _e217 = pos1_;
            let _e218 = tf(_e216, _e217);
            let _e222 = global.U[0];
            let _e225 = inverseTexTransform;
            let _e226 = pos1_;
            let _e227 = tf(_e225, _e226);
            let _e236 = textureSample(t_source, samp, ((vec2<f32>((_e218.x / _e222.x), _e227.y) / vec2(2f)) + vec2(0.5f)));
            let _e237 = inverseTexTransform;
            let _e238 = pos2_;
            let _e239 = tf(_e237, _e238);
            let _e243 = global.U[0];
            let _e246 = inverseTexTransform;
            let _e247 = pos2_;
            let _e248 = tf(_e246, _e247);
            let _e257 = textureSample(t_source, samp, ((vec2<f32>((_e239.x / _e243.x), _e248.y) / vec2(2f)) + vec2(0.5f)));
            let _e258 = k;
            return mix(_e236, _e257, vec4(_e258));
        }
    }
}
