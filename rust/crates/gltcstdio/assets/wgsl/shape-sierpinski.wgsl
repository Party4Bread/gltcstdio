...
fn basicInterpolatedShapes(uv: vec2<f32>, outPos: vec2<f32>, insideImage_specified: i32, iterations: i32, shadows: f32, roundness: f32, multiplier: f32, colorOutline: vec4<f32>, outlineThickness: f32, brightness_2: f32, contrast_2: f32, saturation_2: f32, hue_2: f32, colorIn: vec4<f32>, colorOut: vec4<f32>, colorShadow: vec4<f32>, colorGlow: vec4<f32>, insideLock: i32, modelTransform: mat3x3<f32>, insideTransform: mat3x3<f32>, shadowTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    if ((_e287 == 1i) && _e290) {
        ...
        let _e309 = textureSample(t_insideImage, samp, _e308);
        local_9 = _e309;
    } else {
        let _e310 = v;
        let _e314 = global.U[0];
        let _e317 = v;
        let _e326 = _mirror_wrap(((vec2<f32>((_e310.x / _e314.x), _e317.y) / vec2(2f)) + vec2(0.5f)));
        let _e327 = textureSample(t_source, samp, _e326);
        local_9 = _e327;
    }
    let _e329 = local_9;
    sColor = _e329;
    ...
    if _e333 {
        let _e334 = color;
        let _e335 = brightness_3;
        let _e336 = contrast_3;
        let _e339 = saturation_3;
        let _e340 = hue_3;
        let _e343 = adjustColorHSLuv(_e334, _e335, _e336, 0f, 0f, _e339, _e340, vec4(0f));
        color = _e343;
    }
    let _e344 = colorGlow_1;
    if (_e344.w != 0f) {
        let _e348 = colorGlow_1;
        let _e352 = d;
        let _e355 = ((_e348.xyz * 0.01f) / vec3(abs(_e352)));
        let _e357 = colorGlow_1;
        let _e361 = d;
        local_10 = vec4<f32>(_e355.x, _e355.y, _e355.z, min(1f, ((_e357.w * 0.01f) / abs(_e361))));
    } else {
        local_10 = vec4(0f);
    }
    let _e372 = local_10;
    ...
    let _e374 = color;
    let _e375 = tint_4;
    let _e376 = mergeColor(_e374, _e375);
    ...
    let _e388 = glow_2;
    let _e389 = mergeGlow(_e387, _e388);
    color = _e389;
    let _e390 = d;
    let _e392 = outlineThickness_1;
    if (abs(_e390) < (_e392 * 0.5f)) {
        {
            let _e396 = color;
            let _e397 = colorOutline_1;
            let _e398 = mergeColor(_e396, _e397);
            color = _e398;
            shadow = 0f;
        }
    }
