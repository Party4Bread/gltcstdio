    ...
    if (abs(_e44.y) < 1f) {
        {
            ...
            let _e59 = u;
            p_3 = (_e58 * vec3<f32>(_e59.x, _e59.y, 1f)).xy;
            let _e67 = p_3;
            let _e71 = global.U[0];
            let _e74 = p_3;
            let _e83 = textureSample(t_source, samp, ((vec2<f32>((_e67.x / _e71.x), _e74.y) / vec2(2f)) + vec2(0.5f)));
            let _e84 = color_1;
            let _e85 = breakRectColorize(_e83, _e84);
            outCol = _e85;
            let _e87 = outCol;
            return _e87;
        }
