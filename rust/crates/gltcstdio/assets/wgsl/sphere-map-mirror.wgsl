...
fn sphereMapMirror(pos: vec2<f32>, outPos: vec2<f32>, countX: i32, countY: i32, model3DTransform: mat4x4<f32>) -> vec4<f32> {
    var pos_1: vec2<f32>;
    ...
    let _e69 = longLat;
    let _e71 = countY_1;
    u_1 = vec2<f32>((((-(_e58.x) / 3.1415927f) * 0.5f) * f32(_e65)), (0.5f + ((_e69.y * f32(_e71)) / 3.1415927f)));
    let _e79 = u_1;
    let _e83 = global.U[0];
    let _e86 = u_1;
...
fn main_1() {
    ...
    let _e107 = sphereMapMirror((_naga_inverse_3x3_f32(mat3x3<f32>(vec3<f32>(_e9.x, _e9.y, _e9.z), vec3<f32>(_e13.x, _e13.y, _e13.z), vec3<f32>(_e17.x, _e17.y, _e17.z))) * vec3<f32>(_e44.x, _e44.y, 1f)).xy, (((_e51 - vec2(0.5f)) * 2f) * vec2<f32>(_e59.x, 1f)), i32(_e66.x), i32(_e71.x), mat4x4<f32>(vec4<f32>(_e76.x, _e76.y, _e76.z, _e76.w), vec4<f32>(_e79.x, _e79.y, _e79.z, _e79.w), vec4<f32>(_e82.x, _e82.y, _e82.z, _e82.w), vec4<f32>(_e85.x, _e85.y, _e85.z, _e85.w)));
    fragColor = _e107;
    return;
}
