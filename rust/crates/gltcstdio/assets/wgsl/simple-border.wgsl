...
fn simpleBorder(uv: vec2<f32>, outPos: vec2<f32>, border: f32, sourceDim: vec2<f32>, shadows: f32, outDim: vec2<f32>, colorOut: vec4<f32>, colorShadow: vec4<f32>, viewTransform: mat3x3<f32>, shadowTransform: mat3x3<f32>, modelTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    if _e113 {
        ...
        let _e131 = textureSample(t_source, samp, _e130);
        local_1 = _e131;
    } else {
        let _e132 = v;
        let _e136 = global.U[0];
        let _e139 = v;
        let _e148 = _mirror_wrap(((vec2<f32>((_e132.x / _e136.x), _e139.y) / vec2(2f)) + vec2(0.5f)));
        let _e149 = textureSample(t_source, samp, _e148);
        let _e150 = colorOut_1;
        let _e151 = mergeColor(_e149, _e150);
        local_1 = _e151;
    }
    let _e153 = local_1;
    outCol = _e153;
    let _e155 = outCol;
    let _e156 = outCol;
    let _e157 = colorShadow_1;
    let _e158 = mergeColor(_e156, _e157);
    let _e159 = shadow;
    return mix(_e155, _e158, vec4(_e159));
}
