...
fn basicInterpolatedShapes(uv: vec2<f32>, outPos: vec2<f32>, insideImage_specified: i32, shapeAspectRatio: f32, shadows: f32, roundness: f32, multiplier: f32, colorOutline: vec4<f32>, outlineThickness: f32, brightness_2: f32, contrast_2: f32, saturation_2: f32, hue_2: f32, colorIn: vec4<f32>, colorOut: vec4<f32>, colorShadow: vec4<f32>, colorGlow: vec4<f32>, insideLock: i32, modelTransform: mat3x3<f32>, insideTransform: mat3x3<f32>, shadowTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    if ((_e146 == 1i) && _e149) {
        ...
        let _e168 = textureSample(t_insideImage, samp, _e167);
        local_11 = _e168;
    } else {
        let _e169 = v;
    ...
    if _e192 {
        let _e193 = color;
        let _e194 = brightness_3;
        let _e195 = contrast_3;
        let _e198 = saturation_3;
        let _e199 = hue_3;
        let _e202 = adjustColorHSLuv(_e193, _e194, _e195, 0f, 0f, _e198, _e199, vec4(0f));
        color = _e202;
    }
    let _e203 = colorGlow_1;
    if (_e203.w != 0f) {
        let _e207 = colorGlow_1;
        let _e211 = d;
        let _e214 = ((_e207.xyz * 0.01f) / vec3(abs(_e211)));
        let _e216 = colorGlow_1;
        let _e220 = d;
        local_12 = vec4<f32>(_e214.x, _e214.y, _e214.z, min(1f, ((_e216.w * 0.01f) / abs(_e220))));
    } else {
        local_12 = vec4(0f);
    }
    let _e231 = local_12;
    ...
    let _e233 = color;
    let _e234 = tint_4;
    let _e235 = mergeColor(_e233, _e234);
    let _e236 = colorShadow_1;
    let _e237 = _e236.xyz;
    let _e238 = colorShadow_1;
    let _e240 = shadow;
    let _e246 = mergeColor(_e235, vec4<f32>(_e237.x, _e237.y, _e237.z, (_e238.w * _e240)));
    let _e247 = glow_2;
    let _e248 = mergeGlow(_e246, _e247);
    color = _e248;
    let _e249 = d;
    let _e251 = outlineThickness_1;
    if (abs(_e249) < (_e251 * 0.5f)) {
        {
            let _e255 = color;
            let _e256 = colorOutline_1;
            let _e257 = mergeColor(_e255, _e256);
            color = _e257;
            shadow = 0f;
        }
    }
