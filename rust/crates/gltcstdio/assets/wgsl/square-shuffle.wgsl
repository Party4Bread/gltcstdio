...
fn squareShuffle(uv: vec2<f32>, outPos: vec2<f32>, intensity: f32, angle: f32, randomSeed: f32, modelTransform: mat3x3<f32>) -> vec4<f32> {
    var uv_1: vec2<f32>;
    ...
    if (_e30 != 0f) {
        {
            ...
            let _e55 = indices;
            let _e57 = cosPhase;
            ri = vec2<f32>(floor((((_e39.x * _e41) - (_e43.y * _e45)) + 0.5f)), floor((((_e51.x * _e53) + (_e55.y * _e57)) + 0.5f)));
            let _e65 = ri;
            indices = _e65;
        }
    }
    let _e66 = intensity_1;
    if (_e66 != 0f) {
        {
            let _e69 = indices;
            let _e70 = randomSeed_1;
            let _e71 = rand2relSeeded(_e69, _e70);
            rnd = (_e71 * 20f);
            let _e75 = rnd;
            probability = fract(_e75).x;
            let _e79 = intensity_1;
            let _e80 = probability;
            if (_e79 > _e80) {
                {
                    let _e82 = rnd;
                    delta = ceil(_e82);
    ...
    let _e92 = u_2;
    let _e93 = tf(_e91, _e92);
    coord = _e93;
    let _e95 = coord;
    let _e99 = global.U[0];
    let _e102 = coord;
    let _e111 = textureSample(t_source, samp, ((vec2<f32>((_e95.x / _e99.x), _e102.y) / vec2(2f)) + vec2(0.5f)));
    return _e111;
}
