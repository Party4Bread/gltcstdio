...
fn sdVesica(u: vec2<f32>, r_1: f32, d: f32) -> f32 {
    var u_1: vec2<f32>;
    var r_2: f32;
    var d_1: f32;
    var b_1: f32;
    ...
    if (((_e24.y - _e26) * _e28) > (_e30.x * _e32)) {
        let _e35 = u_1;
        let _e37 = b_1;
        local_8 = length((_e35 - vec2<f32>(0f, _e37)));
    } else {
        let _e41 = u_1;
        let _e42 = d_1;
        let _e48 = r_2;
        local_8 = (length((_e41 - vec2<f32>(-(_e42), 0f))) - _e48);
    }
    let _e51 = local_8;
    return _e51;
}
...
fn basicInterpolatedShapes(uv: vec2<f32>, outPos: vec2<f32>, insideImage_specified: i32, thickness: f32, shadows: f32, roundness: f32, multiplier: f32, colorOutline: vec4<f32>, outlineThickness: f32, brightness_2: f32, contrast_2: f32, saturation_2: f32, hue_2: f32, colorIn: vec4<f32>, colorOut: vec4<f32>, colorShadow: vec4<f32>, colorGlow: vec4<f32>, insideLock: i32, modelTransform: mat3x3<f32>, insideTransform: mat3x3<f32>, shadowTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    let _e62 = sdVesica(_e56, 0.7f, mix(0.7f, 0f, _e60));
    d_2 = _e62;
    let _e63 = d_2;
            ...
            if (_e80 < 0f) {
                {
                    ...
                    let _e100 = sdVesica(_e94, 0.7f, mix(0.7f, 0f, _e98));
                    d_2 = _e100;
                    let _e102 = shadows_1;
                    let _e104 = d_2;
            ...
            if (_e123 > 0f) {
                {
                    ...
                    d_2 = ((_e132 * _e133) - _e135);
                    let _e137 = u_4;
                    let _e141 = thickness_1;
                    let _e143 = sdVesica(_e137, 0.7f, mix(0.7f, 0f, _e141));
                    d_2 = _e143;
                    let _e145 = shadows_1;
                    let _e147 = d_2;
                    shadow = (0.7f * smoothstep(_e145, 0f, _e147));
                    let _e150 = saveD_1;
                    d_2 = _e150;
                }
            }
            let _e151 = colorOut_1;
            tint_4 = _e151;
        }
    }
    let _e152 = insideImage_specified_1;
    let _e155 = inside;
    if ((_e152 == 1i) && _e155) {
        let _e157 = v;
        let _e161 = global.U[0];
        let _e164 = v;
        let _e173 = _mirror_wrap(((vec2<f32>((_e157.x / _e161.x), _e164.y) / vec2(2f)) + vec2(0.5f)));
        let _e174 = textureSample(t_insideImage, samp, _e173);
        local_10 = _e174;
    } else {
        let _e175 = v;
        let _e179 = global.U[0];
        let _e182 = v;
        let _e191 = _mirror_wrap(((vec2<f32>((_e175.x / _e179.x), _e182.y) / vec2(2f)) + vec2(0.5f)));
        let _e192 = textureSample(t_source, samp, _e191);
        local_10 = _e192;
    }
    let _e194 = local_10;
    sColor = _e194;
    ...
    if _e198 {
        let _e199 = color;
        let _e200 = brightness_3;
        let _e201 = contrast_3;
        let _e204 = saturation_3;
        let _e205 = hue_3;
        let _e208 = adjustColorHSLuv(_e199, _e200, _e201, 0f, 0f, _e204, _e205, vec4(0f));
        color = _e208;
    }
    ...
    if (_e209.w != 0f) {
        ...
        let _e217 = d_2;
        let _e220 = ((_e213.xyz * 0.01f) / vec3(abs(_e217)));
        let _e222 = colorGlow_1;
        let _e226 = d_2;
        local_11 = vec4<f32>(_e220.x, _e220.y, _e220.z, min(1f, ((_e222.w * 0.01f) / abs(_e226))));
    } else {
        local_11 = vec4(0f);
    }
    let _e237 = local_11;
    ...
    let _e239 = color;
    let _e240 = tint_4;
    let _e241 = mergeColor(_e239, _e240);
    let _e242 = colorShadow_1;
    let _e243 = _e242.xyz;
    let _e244 = colorShadow_1;
    let _e246 = shadow;
    let _e252 = mergeColor(_e241, vec4<f32>(_e243.x, _e243.y, _e243.z, (_e244.w * _e246)));
    let _e253 = glow_2;
    let _e254 = mergeGlow(_e252, _e253);
    color = _e254;
    let _e255 = d_2;
    let _e257 = outlineThickness_1;
    if (abs(_e255) < (_e257 * 0.5f)) {
        {
            let _e261 = color;
            let _e262 = colorOutline_1;
            let _e263 = mergeColor(_e261, _e262);
            color = _e263;
            shadow = 0f;
        }
    }
