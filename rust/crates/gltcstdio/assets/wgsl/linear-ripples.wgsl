...
fn linearRipples(pos: vec2<f32>, outPos: vec2<f32>, intensity: f32, perspective: f32, modelTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    let _e54 = perspective_1;
    let _e55 = radius;
    p = (_e54 * _e55);
    let _e58 = perspective_1;
    if (_e58 == 0f) {
        local_1 = 0f;
    } else {
        let _e62 = perspective_1;
        if (_e62 >= 10000f) {
            let _e65 = d;
            local = _e65;
        } else {
    ...
    let _e83 = pd;
    let _e88 = radius;
    dilation = (((_e78 * _e79) * 0.5f) * sin((((_e83 * 3.1415927f) * 100f) / _e88)));
    let _e93 = modelTransform_1;
    let _e94 = u;
    let _e96 = u;
    let _e98 = dilation;
    coord = (_e93 * vec3<f32>(_e94.x, (_e96.y + _e98), 1f)).xy;
    let _e105 = coord;
