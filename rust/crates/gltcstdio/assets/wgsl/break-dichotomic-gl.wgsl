...
fn breakDichotomic(pos_2: vec2<f32>, outPos: vec2<f32>, mode_2: i32, count: i32, intensity_2: f32, randomSeed: f32, regularity: f32, thickness: f32, color: vec4<f32>, sourceDim: vec2<f32>, modelTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    let _e320 = pos_3;
    let _e329 = textureSample(t_source, samp, ((vec2<f32>((_e313.x / _e317.x), _e320.y) / vec2(2f)) + vec2(0.5f)));
    col = _e329;
    let _e332 = border;
    if _e332 {
        {
            ...
            let _e335 = color_1;
            let _e337 = color_1;
            let _e340 = mix(_e333.xyz, _e335.xyz, vec3(_e337.w));
            let _e341 = col;
            outCol = vec4<f32>(_e340.x, _e340.y, _e340.z, _e341.w);
        }
    } else {
        {
            let _e347 = p;
            let _e351 = global.U[0];
            let _e354 = p;
            let _e363 = textureSample(t_source, samp, ((vec2<f32>((_e347.x / _e351.x), _e354.y) / vec2(2f)) + vec2(0.5f)));
            outCol = _e363;
        }
    }
    let _e364 = outCol;
