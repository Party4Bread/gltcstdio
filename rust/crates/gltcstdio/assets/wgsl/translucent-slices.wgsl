...
fn translucentSlices(pos: vec2<f32>, outPos: vec2<f32>, intensity: f32, count: i32, overlap: f32, dampening: f32, mode: i32, model3DTransform: mat4x4<f32>, sourceDim: vec2<f32>) -> vec4<f32> {
    ...
    loop {
        ...
        }
        {
                    ...
                    if (!(_e197) || ((abs(_e199.x) < _e202) && (abs(_e204.y) < 1f))) {
                        {
                            ...
                            let _e218 = uv;
                            let _e227 = textureSample(t_source, samp, ((vec2<f32>((_e211.x / _e215.x), _e218.y) / vec2(2f)) + vec2(0.5f)));
                            sampleCol = _e227;
                            let _e229 = sampleCol;
                            ...
                            if ((_e242 >= _e243) && (_e245 <= _e246)) {
                                {
                                    let _e249 = i;
                                    let _e253 = N;
                                    lumCenter = ((f32(_e249) + 0.5f) / _e253);
                                    let _e256 = lum;
                                    let _e257 = lumCenter;
                                    lDist = abs((_e256 - _e257));
                                    let _e261 = lDist;
                                    let _e262 = layerOpaqueSize;
                                    if (_e261 <= _e262) {
                                        {
                                            let _e264 = col;
                                            let _e265 = sampleCol;
                                            let _e266 = mergeColorOpacifying(_e264, _e265);
                                            col = _e266;
                                        }
                                    } else {
                                        {
                                            let _e269 = lDist;
                                            let _e270 = layerOpaqueSize;
                                            let _e272 = maxDist;
                                            let _e277 = dampening_1;
                                            ka = pow(max(0f, (1f - ((_e269 - _e270) / _e272))), pow(10f, _e277));
                                            let _e281 = col;
                                            let _e282 = sampleCol;
                                            let _e283 = _e282.xyz;
                                            let _e284 = sampleCol;
                                            let _e286 = ka;
                                            let _e292 = mergeColorOpacifying(_e281, vec4<f32>(_e283.x, _e283.y, _e283.z, (_e284.w * _e286)));
                                            col = _e292;
                                        }
                                    }
                                    let _e293 = col;
            ...
            let _e305 = iterDir;
            let _e307 = i;
            if ((_e300 && (_e301 >= _e302)) || (!(_e305) && (_e307 < 0i))) {
                break;
            }
        }
    }
    let _e312 = dir_2;
    let _e313 = getBackground(_e312);
    let _e314 = col;
    let _e315 = mergeColorOpacifying(_e313, _e314);
    col = _e315;
    let _e316 = col;
    let _e317 = _e316.xyz;
    return vec4<f32>(_e317.x, _e317.y, _e317.z, 1f);
}
