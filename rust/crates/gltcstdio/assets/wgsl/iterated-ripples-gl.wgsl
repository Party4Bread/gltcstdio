...
fn iteratedRipplesGL(pos: vec2<f32>, outPos: vec2<f32>, intensity: f32, dampening: f32, count: i32, modelTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    let _e38 = textureSample(t_source, samp, _e37);
    color = _e38;
    let _e40 = invM;
    let _e41 = pos_1;
    u = (_e40 * vec3<f32>(_e41.x, _e41.y, 1f)).xy;
    let _e49 = count_1;
    rippleCount = f32(_e49);
    loop {
        let _e54 = i;
        if !((_e54 < 6i)) {
            break;
        }
        {
            ...
            if (_e64 >= 1f) {
                ...
                {
                    let _e68 = dampening_1;
                    if (_e68 >= 0f) {
                        let _e72 = d;
                        let _e74 = dampening_1;
                        local = pow((1f - _e72), (_e74 * 2f));
                    } else {
                        let _e78 = d;
                        let _e79 = dampening_1;
                        local = pow(_e78, (-(_e79) * 5f));
                    }
                    let _e85 = local;
                    dampen = _e85;
                    let _e88 = intensity_1;
                    let _e89 = d;
                    let _e90 = rippleCount;
                    let _e96 = dampen;
                    dilation = (1f + ((_e88 * sin(((_e89 * _e90) * 3.1415927f))) * _e96));
                    let _e100 = u;
                    let _e101 = dilation;
                    u = (_e100 * _e101);
                }
    ...
    let _e109 = u;
    u = (_e108 * vec3<f32>(_e109.x, _e109.y, 1f)).xy;
    let _e116 = u;
