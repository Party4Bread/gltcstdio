...
fn basicInterpolatedShapes(uv: vec2<f32>, outPos: vec2<f32>, insideImage_specified: i32, shadows: f32, roundness: f32, multiplier: f32, colorOutline: vec4<f32>, outlineThickness: f32, brightness_2: f32, contrast_2: f32, saturation_2: f32, hue_2: f32, colorIn: vec4<f32>, colorOut: vec4<f32>, colorShadow: vec4<f32>, colorGlow: vec4<f32>, insideLock: i32, modelTransform: mat3x3<f32>, insideTransform: mat3x3<f32>, shadowTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    if ((_e138 == 1i) && _e141) {
        ...
        let _e160 = textureSample(t_insideImage, samp, _e159);
        local_9 = _e160;
    } else {
        let _e161 = v;
    ...
    if _e184 {
        let _e185 = color;
        let _e186 = brightness_3;
        let _e187 = contrast_3;
        let _e190 = saturation_3;
        let _e191 = hue_3;
        let _e194 = adjustColorHSLuv(_e185, _e186, _e187, 0f, 0f, _e190, _e191, vec4(0f));
        color = _e194;
    }
    let _e195 = colorGlow_1;
    if (_e195.w != 0f) {
        let _e199 = colorGlow_1;
        let _e203 = d;
        let _e206 = ((_e199.xyz * 0.01f) / vec3(abs(_e203)));
        let _e208 = colorGlow_1;
        let _e212 = d;
        local_10 = vec4<f32>(_e206.x, _e206.y, _e206.z, min(1f, ((_e208.w * 0.01f) / abs(_e212))));
    } else {
        local_10 = vec4(0f);
    }
    let _e223 = local_10;
    ...
    let _e228 = colorShadow_1;
    let _e229 = _e228.xyz;
    let _e230 = colorShadow_1;
    let _e232 = shadow;
    let _e238 = mergeColor(_e227, vec4<f32>(_e229.x, _e229.y, _e229.z, (_e230.w * _e232)));
    let _e239 = glow_2;
    let _e240 = mergeGlow(_e238, _e239);
    color = _e240;
    let _e241 = d;
    let _e243 = outlineThickness_1;
    if (abs(_e241) < (_e243 * 0.5f)) {
        {
            let _e247 = color;
            let _e248 = colorOutline_1;
            let _e249 = mergeColor(_e247, _e248);
            color = _e249;
            shadow = 0f;
        }
    }
