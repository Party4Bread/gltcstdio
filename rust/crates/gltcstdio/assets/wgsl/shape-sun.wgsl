...
fn basicInterpolatedShapes(uv: vec2<f32>, outPos: vec2<f32>, insideImage_specified: i32, shadows: f32, roundness: f32, multiplier: f32, colorOutline: vec4<f32>, outlineThickness: f32, brightness_2: f32, contrast_2: f32, saturation_2: f32, hue_2: f32, colorIn: vec4<f32>, colorOut: vec4<f32>, colorShadow: vec4<f32>, colorGlow: vec4<f32>, insideLock: i32, modelTransform: mat3x3<f32>, insideTransform: mat3x3<f32>, shadowTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    if ((_e177 == 1i) && _e180) {
        ...
        let _e199 = textureSample(t_insideImage, samp, _e198);
        local_9 = _e199;
    } else {
        let _e200 = v;
        ...
        let _e217 = textureSample(t_source, samp, _e216);
        local_9 = _e217;
    }
    let _e219 = local_9;
    sColor = _e219;
    ...
    if _e223 {
        let _e224 = color;
        let _e225 = brightness_3;
        let _e226 = contrast_3;
        let _e229 = saturation_3;
        let _e230 = hue_3;
        let _e233 = adjustColorHSLuv(_e224, _e225, _e226, 0f, 0f, _e229, _e230, vec4(0f));
        color = _e233;
    }
    let _e234 = colorGlow_1;
    if (_e234.w != 0f) {
        let _e238 = colorGlow_1;
        let _e242 = d;
        let _e245 = ((_e238.xyz * 0.01f) / vec3(abs(_e242)));
        let _e247 = colorGlow_1;
        let _e251 = d;
        local_10 = vec4<f32>(_e245.x, _e245.y, _e245.z, min(1f, ((_e247.w * 0.01f) / abs(_e251))));
    } else {
        local_10 = vec4(0f);
    }
    let _e262 = local_10;
    ...
    let _e267 = colorShadow_1;
    let _e268 = _e267.xyz;
    let _e269 = colorShadow_1;
    let _e271 = shadow;
    let _e277 = mergeColor(_e266, vec4<f32>(_e268.x, _e268.y, _e268.z, (_e269.w * _e271)));
    let _e278 = glow_2;
    let _e279 = mergeGlow(_e277, _e278);
    color = _e279;
    let _e280 = d;
    let _e282 = outlineThickness_1;
    if (abs(_e280) < (_e282 * 0.5f)) {
        {
            let _e286 = color;
            let _e287 = colorOutline_1;
            let _e288 = mergeColor(_e286, _e287);
            color = _e288;
            shadow = 0f;
        }
    }
