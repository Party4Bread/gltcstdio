    ...
    if (_e134 > 0f) {
        {
            ...
            reflectedDir = reflect(_e158, _e159);
            {
                ...
                let _e267 = textureSample(t_source, samp, _e266);
                reflectedColor = _e267;
            }
            {
                let _e269 = refractedDir;
                _n_1 = normalize(_e269);
                let _e272 = _n_1;
                let _e273 = projEquirectangular(_e272);
                _ll_1 = _e273;
                let _e279 = _ll_1;
                let _e286 = _nX_1;
                let _e289 = _ll_1;
                let _e291 = _nY_1;
                _u_1 = vec2<f32>((((-(_e279.x) / 3.1415927f) * 0.5f) * _e286), (0.5f + ((_e289.y * _e291) / 3.1415927f)));
                let _e298 = _u_1;
                _xa_1 = abs(_e298.x);
                let _e302 = _xa_1;
                let _e304 = _xa_1;
                _xa_1 = (_e302 - (2f * floor((_e304 * 0.5f))));
                let _e310 = _xa_1;
                if (_e310 > 1f) {
                    let _e314 = _xa_1;
                    local_3 = (2f - _e314);
                ...
                _x_1 = _e318;
                let _e320 = _u_1;
                _y_1 = clamp(_e320.y, 0f, 1f);
                let _e326 = sourceDim_1;
                let _e328 = sourceDim_1;
                _ratio_1 = (_e326.x / _e328.y);
                let _e332 = _x_1;
                let _e337 = _ratio_1;
                let _e339 = _y_1;
                ...
                let _e356 = _ratio_1;
                let _e358 = _y_1;
                let _e372 = _mirror_wrap(((vec2<f32>((vec2<f32>((((_e332 - 0.5f) * 2f) * _e337), ((_e339 - 0.5f) * 2f)).x / _e348.x), vec2<f32>((((_e351 - 0.5f) * 2f) * _e356), ((_e358 - 0.5f) * 2f)).y) / vec2(2f)) + vec2(0.5f)));
                let _e373 = textureSample(t_source, samp, _e372);
                refractedColor = _e373;
            }
            let _e374 = reflectedColor;
            let _e375 = refractedColor;
            let _e376 = incidence;
            let _e377 = balance_1;
            result = mix(_e374, _e375, vec4(clamp((_e376 + _e377), 0f, 1f)));
        }
    } else {
        {
            {
                let _e385 = dir_2;
                _n_2 = normalize(_e385);
                let _e388 = _n_2;
                let _e389 = projEquirectangular(_e388);
                _ll_2 = _e389;
                let _e395 = _ll_2;
                let _e402 = _nX_2;
                let _e405 = _ll_2;
                let _e407 = _nY_2;
                _u_2 = vec2<f32>((((-(_e395.x) / 3.1415927f) * 0.5f) * _e402), (0.5f + ((_e405.y * _e407) / 3.1415927f)));
                let _e414 = _u_2;
                _xa_2 = abs(_e414.x);
                let _e418 = _xa_2;
                let _e420 = _xa_2;
                _xa_2 = (_e418 - (2f * floor((_e420 * 0.5f))));
                let _e426 = _xa_2;
                if (_e426 > 1f) {
                    let _e430 = _xa_2;
                    local_4 = (2f - _e430);
                ...
                _x_2 = _e434;
                let _e436 = _u_2;
                _y_2 = clamp(_e436.y, 0f, 1f);
                let _e442 = sourceDim_1;
                let _e444 = sourceDim_1;
                _ratio_2 = (_e442.x / _e444.y);
                let _e448 = _x_2;
                let _e453 = _ratio_2;
                let _e455 = _y_2;
                ...
                let _e472 = _ratio_2;
                let _e474 = _y_2;
                let _e488 = _mirror_wrap(((vec2<f32>((vec2<f32>((((_e448 - 0.5f) * 2f) * _e453), ((_e455 - 0.5f) * 2f)).x / _e464.x), vec2<f32>((((_e467 - 0.5f) * 2f) * _e472), ((_e474 - 0.5f) * 2f)).y) / vec2(2f)) + vec2(0.5f)));
                let _e489 = textureSample(t_source, samp, _e488);
                bkg = _e489;
            }
            let _e490 = bkg;
            result = _e490;
