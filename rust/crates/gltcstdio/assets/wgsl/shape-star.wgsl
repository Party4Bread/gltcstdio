...
fn basicInterpolatedShapes(uv: vec2<f32>, outPos: vec2<f32>, insideImage_specified: i32, spikeCount_2: i32, radius: f32, shadows: f32, roundness: f32, multiplier: f32, colorOutline: vec4<f32>, outlineThickness: f32, brightness_2: f32, contrast_2: f32, saturation_2: f32, hue_2: f32, colorIn: vec4<f32>, colorOut: vec4<f32>, colorShadow: vec4<f32>, colorGlow: vec4<f32>, insideLock: i32, modelTransform: mat3x3<f32>, insideTransform: mat3x3<f32>, shadowTransform: mat3x3<f32>) -> vec4<f32> {
    ...
    if ((_e184 == 1i) && _e187) {
        ...
        let _e206 = textureSample(t_insideImage, samp, _e205);
        local_9 = _e206;
    } else {
        let _e207 = v;
        ...
        let _e224 = textureSample(t_source, samp, _e223);
        local_9 = _e224;
    }
    let _e226 = local_9;
    sColor = _e226;
    ...
    if _e230 {
        let _e231 = color;
        let _e232 = brightness_3;
        let _e233 = contrast_3;
        let _e236 = saturation_3;
        let _e237 = hue_3;
        let _e240 = adjustColorHSLuv(_e231, _e232, _e233, 0f, 0f, _e236, _e237, vec4(0f));
        color = _e240;
    }
    let _e241 = colorGlow_1;
    if (_e241.w != 0f) {
        let _e245 = colorGlow_1;
        let _e249 = d;
        let _e252 = ((_e245.xyz * 0.01f) / vec3(abs(_e249)));
        let _e254 = colorGlow_1;
        let _e258 = d;
        local_10 = vec4<f32>(_e252.x, _e252.y, _e252.z, min(1f, ((_e254.w * 0.01f) / abs(_e258))));
    } else {
        local_10 = vec4(0f);
    }
    let _e269 = local_10;
    ...
    let _e271 = color;
    let _e272 = tint_4;
    let _e273 = mergeColor(_e271, _e272);
    let _e274 = colorShadow_1;
    let _e275 = _e274.xyz;
    let _e276 = colorShadow_1;
    let _e278 = shadow;
    let _e284 = mergeColor(_e273, vec4<f32>(_e275.x, _e275.y, _e275.z, (_e276.w * _e278)));
    let _e285 = glow_2;
    let _e286 = mergeGlow(_e284, _e285);
    color = _e286;
    let _e287 = d;
    let _e289 = outlineThickness_1;
    if (abs(_e287) < (_e289 * 0.5f)) {
        {
            let _e293 = color;
            let _e294 = colorOutline_1;
            let _e295 = mergeColor(_e293, _e294);
            color = _e295;
            shadow = 0f;
        }
    }
