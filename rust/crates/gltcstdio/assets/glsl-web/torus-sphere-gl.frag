...
#define u_objectColor (U[13])
#define u_glowColor (U[14])
#define u_bkgColor (U[15])
#define u_backgroundStyle (int(U[16].x))
...
vec3 torusSphereRayMarch(vec3 origin, vec3 dir, float radius, vec4 glowColor) {
    ...
    // Pap's 0.5*(1 + 1.25 + u_Radius*0.02) becomes 0.5*(2.25 + radius) in gltcstdio units.
    float minDist = 1e9;
    float k = 0.0;
...
void main() {
    fragColor = torusSphereGl((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_model3DTransform, u_sourceDim, u_intensity, u_reflectivity, u_radius, u_objectColor, u_glowColor, u_bkgColor, u_backgroundStyle);
}
