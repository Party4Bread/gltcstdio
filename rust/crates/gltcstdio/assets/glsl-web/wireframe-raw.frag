...
#define u_transparencyMode (int(U[18].x))
#define u_colorBkg (U[19])
#define u_colorLines (U[20])
#define u_colorFog (U[21])
...
void main() {
    fragColor = wireframe((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_intensity, u_mode, u_rezolution, u_thickness, u_glow, u_model3DTransform, u_sourceDim, u_sourceElevationDim, u_sourceBkg_specified, u_sourceElevation_specified, u_transparencyMode, u_colorBkg, u_colorLines, u_colorFog);
}
