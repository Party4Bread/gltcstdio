...
float voronoiBoundary(vec2 coord, float k, float border, float seed) {
    ...
    return (minD2-minD)<border ? 1.0 : 0.0;
}

vec4 cristalBorder(vec2 pos, vec2 outPos, float border, float thickness, vec2 sourceDim, vec2 outDim, vec4 borderColor, float variability, float randomSeed, mat3 modelTransform, mat3 borderTransform) {
    ...
    float k = 1.0-voronoiBoundary((inverse(borderTransform) * vec3(pos, 1.0)).xy, 200.0, thickness*2.0*B, randomSeed);
...
void main() {
    fragColor = cristalBorder((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_border, u_thickness, u_sourceDim, u_outDim, u_borderColor, u_variability, u_randomSeed, u_modelTransform, u_borderTransform);
}
