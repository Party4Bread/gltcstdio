...
float implicitFn(vec3 p, int spheres_size) {
    float total = 0.0;
...
vec3 getIntersectionD(vec3 origin, vec3 dir, float sphereRad, int spheres_size) {
    float minDist = 1e9;
    ...
    float originSign = sign(implicitFn(origin, spheres_size));
    float steps = 100.0;
    float dk = (kBounds.y - k0) / steps;
    vec3 x0 = origin + k0 * dir;
    vec3 x1 = x0;
    float a = implicitFn(x0, spheres_size);
    float b = a;
    ...
    while (iter < maxSecantIter) {
        ...
        float c = implicitFn(x2, spheres_size);
...
vec3 getNormal(vec3 p, int spheres_size) {
    float d = 0.01;
    float d2 = d * 2.0;
    return normalize(vec3(
        (implicitFn(vec3(p.x - d, p.y, p.z), spheres_size) - implicitFn(vec3(p.x + d, p.y, p.z), spheres_size)) / d2,
        (implicitFn(vec3(p.x, p.y - d, p.z), spheres_size) - implicitFn(vec3(p.x, p.y + d, p.z), spheres_size)) / d2,
        (implicitFn(vec3(p.x, p.y, p.z - d), spheres_size) - implicitFn(vec3(p.x, p.y, p.z + d), spheres_size)) / d2
    ));
}

        vec4 metaballs3d(vec2 pos, vec2 outPos, mat4 model3DTransform, vec2 sourceDim, float intensity, float reflectivity, vec4 objectColor, vec4 glowColor, vec4 bkgColor, int backgroundStyle, int spheres_size) {
            ...
            do {
                ...
                vec3 inters = getIntersectionD(origin, dir, sphereRad, spheres_size);
                float k = inters.x;
                if (k > 0.0 && k < minK) {
                    minK = k;
                    minI = 0;
                ...
                if (minI >= 0) {
                    ...
                    vec3 normal = implicitFn(origin, spheres_size) <= 0.0 ? getNormal(intersection, spheres_size) : -getNormal(intersection, spheres_size);
                    if (iter == maxIter) {
...
void main() {
    fragColor = metaballs3d((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_model3DTransform, u_sourceDim, u_intensity, u_reflectivity, u_objectColor, u_glowColor, u_bkgColor, u_backgroundStyle, u_spheres_size);
}
