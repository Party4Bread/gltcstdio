...
vec4 transparentCube(vec2 pos, vec2 outPos, float intensity, float dampening, mat4 model3DTransform, vec2 sourceDim) {
    vec4 backgroundColor = vec4(0.0, 0.0, 0.0, 1.0);

    // Normalize dampening from -1..1 to -100..100
    float normalizedDampening = dampening * 100.0;
    ...
    // March along the ray and accumulate colors
    float k = k1;
    ...
    if (normalizedDampening==0.0) {
        ...
        // Weighted averaging based on dampening
        float weight = 1.0;
        float dw = pow(1.001-normalizedDampening*0.01, 10.0/max(sourceDim.x, sourceDim.y));
        do {
            prevColor = color;
            prevDz = dz;
            prevH = h;
...
void main() {
    fragColor = transparentCube((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_intensity, u_dampening, u_model3DTransform, u_sourceDim);
}
