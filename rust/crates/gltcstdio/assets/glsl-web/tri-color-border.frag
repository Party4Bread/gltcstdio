...
#define u_offset (U[8].x)
#define u_shadows (U[9].x)
#define u_color1 (U[10])
#define u_color2 (U[11])
#define u_color3 (U[12])
#define u_colorShadow (U[13])
#define u_shadowTransform (mat3(U[14].xyz, U[15].xyz, U[16].xyz))
...
void main() {
    fragColor = triColorBorder((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_border, u_thickness, u_offset, u_sourceDim, u_shadows, u_outDim, u_color1, u_color2, u_color3, u_colorShadow, u_viewTransform, u_shadowTransform, u_modelTransform);
}
