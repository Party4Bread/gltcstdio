...
#define u_count (int(U[32].x))
#define u_radius (U[33].x)
#define u_randomSeed (U[34].x)
#define u_separation (U[35].x)
...
float sdf(vec3 p, int count, float radius, float randomSeed, float separation) {
    float r = radius;
    float d = 1.0/length(p) - 1.0/r;
    for(int i=1; i<count; ++i) {
        vec3 center = separation * rand13relSeeded(float(i), randomSeed);
        d += 1.0/length(center-p) - 1.0/r;
    }
    return d;
}

vec3 normal(vec3 p, int count, float radius, float randomSeed, float separation) {
    float d = 0.0001;
    float s = sdf(p,count,radius,randomSeed,separation);
    return normalize(vec3(
        (s-sdf(vec3(p.x-d, p.y, p.z),count,radius,randomSeed,separation))/d,
        (s-sdf(vec3(p.x, p.y-d, p.z),count,radius,randomSeed,separation))/d,
        (s-sdf(vec3(p.x, p.y, p.z-d),count,radius,randomSeed,separation))/d
        ));
}

vec3 rayMarch(vec3 p0, vec3 dir, float side, int count, float radius, float randomSeed, float separation) {
    float d = sdf(p0,count,radius,randomSeed,separation);
    float s = sign(d);
    float totalD = 0.0;
    int step = 0;
    while (step < 1000 && d<100.) {
        ...
        d = sdf(p,count,radius,randomSeed,separation);
    ...
    return vec3(INF);
        ...
        vec4 rayMarcher(vec2 uv, vec2 outPos, mat4 model3DTransform, vec2 sourceDim, mat4 lightSourceTransform, mat4 bkgTransform,
                        ...
                        vec4 sourceColor, vec4 ambientColor, float specular, int backgroundStyle, int count, float radius, float randomSeed, float separation) {
            float D = 1.0;
...
//camDir = mat3(invModelTransform) * camDir; 
            ...
            vec3 qIn = rayMarch(camera, camDir, 1.,count,radius,randomSeed,separation);
            ...
            if (qIn.x!=INF) {
                vec3 nIn = normal(qIn,count,radius,randomSeed,separation);
                ...
                if (fresnel!=1.0) {
                    ...
                    vec3 qOut = rayMarch(qIn-nIn*0.001, refractDir, -1.,count,radius,randomSeed,separation);

                    vec3 n = -normal(qOut,count,radius,randomSeed,separation);
...
refractDirB = model3DTransform3 * refractDirB;
    ...
    colB = vec4((refractDirB)*0.5+0.5, 1.0);
                ...
                if (fresnel!=0.0 || specular!=0.0) {
                    ...
                    vec3 qR = rayMarch(qIn+nIn*0.001, reflectDir, 1.,count,radius,randomSeed,separation);
                    if (qR.x!=INF) {
                         vec3 n = normal(qR,count,radius,randomSeed,separation);
...
void main() {
    fragColor = rayMarcher((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_model3DTransform, u_sourceDim, u_lightSourceTransform, u_bkgTransform, u_camera3DTransform, u_colorMaterial, u_refractionIndex, u_fresnelStrength, u_chromaticAberration, u_colorFog, u_sourceColor, u_ambientColor, u_specular, u_backgroundStyle, u_count, u_radius, u_randomSeed, u_separation);
}
