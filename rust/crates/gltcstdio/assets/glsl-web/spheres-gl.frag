...
float sphereHitDist(vec3 center, float radius, vec3 origin, vec3 dir) {
    ...
    return -1.0;
        ...
        vec4 spheresGl(vec2 pos, vec2 outPos, mat4 model3DTransform, vec2 sourceDim, float intensity, float reflectivity, vec4 objectColor, vec4 glowColor, vec4 bkgColor, int backgroundStyle, int spheres_size) {
            ...
            do {
                ...
                for (int i = 0; i < spheres_size; ++i) {
                    float k = sphereHitDist(u_spheres[i].xyz, u_spheres[i].a, origin, dir);
                    if (k > 0.0 && k < minK) { minK = k; minI = i; }
                    else {
    ...
    vec2 _o_pos = vec2(-_o_alpha / PI, 2.0 * _o_beta / PI) * 2.0 - vec2(0.0, 0.0);
    ...
    _reflBkg = vec4((reflectedDir) * 0.5 + 0.5, 1.0);
                        ...
                        reflectedColor = _reflBkg * mix(vec4(1.0), vec4(2.0 * objectColor.rgb, 1.0), objectColor.a);
                    }
    ...
    _bkg = vec4((dir) * 0.5 + 0.5, 1.0);
            ...
            vec4 col = _bkg * mix(vec4(1.0), vec4(2.0 * bkgColor.rgb, 1.0), bkgColor.a)
                     + vec4(glowColor.rgb * 0.2 / pow(minDist, 1.5), 0.0) * glowColor.a;
            return mix(reflectedColor, col, clamp(incidence + balance, 0.0, 1.0));
        }

void main() {
    fragColor = spheresGl((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_model3DTransform, u_sourceDim, u_intensity, u_reflectivity, u_objectColor, u_glowColor, u_bkgColor, u_backgroundStyle, u_spheres_size);
}
