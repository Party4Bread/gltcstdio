...
vec3 sphereIntersection(vec3 center, float radius, vec3 origin, vec3 dir) {
    ...
    return vec3(INF);
        ...
        vec4 sphereGl(vec2 pos, vec2 outPos, mat4 model3DTransform, vec2 sourceDim,
            ...
            float radius = 0.5;
            vec3 intersection = sphereIntersection(vec3(0.0), radius, cameraPos, dir);
    ...
    reflectedColor = vec4((reflectedDir) * 0.5 + 0.5, 1.0);
                ...
                vec3 intersection2 = sphereIntersection(vec3(0.0), radius, intersection + refractedDir * 0.00001, refractedDir);
    ...
    bkg = vec4((dir) * 0.5 + 0.5, 1.0);
                ...
                return bkg * mix(vec4(1.0), vec4(2.0 * bkgColor.rgb, 1.0), bkgColor.a)
                    + vec4(glowColor.rgb * 0.2 / pow(minDist, clamp(minDist, 1.0, 3.0)), 0.0) * glowColor.a;
            }
        }

void main() {
    fragColor = sphereGl((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_model3DTransform, u_sourceDim, u_intensity, u_reflectivity, u_objectColor, u_glowColor, u_bkgColor, u_backgroundStyle);
}
