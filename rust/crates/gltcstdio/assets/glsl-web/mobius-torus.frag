...
#define u_radius (U[32].x)
#define u_count (int(U[33].x))
#define u_roundness (U[34].x)
#define u_angle (U[35].x)
#define u_separation (U[36].x)
...
float sdf(vec3 p, float radius, int count, float roundness, float angle, float separation) {
    float R = 0.5;
    float r = R*radius;
    float a = sqrt(p.x*p.x + p.y*p.y) - R;
    float ang = angle + atan(p.y, p.x) *0.5 * (float(count)-1.0);
    float ca = cos(ang);
    float sa = sin(ang);
    mat2 rot = mat2(ca, sa, sa, -ca);
    vec2 q = rot * vec2(a, p.z);
    if (q.x<0.0) q = -q;
    vec2 c1 = vec2(separation, 0.);
    vec2 d = abs(q-c1)-r;
    return 0.4*(length(max(d,0.0)) + min(max(d.x,d.y),0.0)) - roundness; 
    //return 0.14*(length(max(q,0.0)) + min(max(q.x,q.y),0.0)); // interesting results using q instead of d but messy.
}

vec3 normal(vec3 p, float radius, int count, float roundness, float angle, float separation) {
    float d = 0.0001;
    float s = sdf(p,radius,count,roundness,angle,separation);
    return normalize(vec3(
        (s-sdf(vec3(p.x-d, p.y, p.z),radius,count,roundness,angle,separation))/d,
        (s-sdf(vec3(p.x, p.y-d, p.z),radius,count,roundness,angle,separation))/d,
        (s-sdf(vec3(p.x, p.y, p.z-d),radius,count,roundness,angle,separation))/d
        ));
}

vec3 rayMarch(vec3 p0, vec3 dir, float side, float radius, int count, float roundness, float angle, float separation) {
    float d = sdf(p0,radius,count,roundness,angle,separation);
    float s = sign(d);
    float totalD = 0.0;
    int step = 0;
    while (step < 1000 && d<100.) {
        ...
        d = sdf(p,radius,count,roundness,angle,separation);
    ...
    return vec3(INF);
        ...
        vec4 rayMarcher(vec2 uv, vec2 outPos, mat4 model3DTransform, vec2 sourceDim, mat4 lightSourceTransform, mat4 bkgTransform,
                        ...
                        vec4 sourceColor, vec4 ambientColor, float specular, int backgroundStyle, float radius, int count, float roundness, float angle, float separation) {
            float D = 1.0;
...
//camDir = mat3(invModelTransform) * camDir; 
            ...
            vec3 qIn = rayMarch(camera, camDir, 1.,radius,count,roundness,angle,separation);
            ...
            if (qIn.x!=INF) {
                vec3 nIn = normal(qIn,radius,count,roundness,angle,separation);
                ...
                if (fresnel!=1.0) {
                    ...
                    vec3 qOut = rayMarch(qIn-nIn*0.001, refractDir, -1.,radius,count,roundness,angle,separation);

                    vec3 n = -normal(qOut,radius,count,roundness,angle,separation);
...
refractDirB = model3DTransform3 * refractDirB;
    ...
    colB = vec4((refractDirB)*0.5+0.5, 1.0);
                ...
                if (fresnel!=0.0 || specular!=0.0) {
                    ...
                    vec3 qR = rayMarch(qIn+nIn*0.001, reflectDir, 1.,radius,count,roundness,angle,separation);
                    if (qR.x!=INF) {
                         vec3 n = normal(qR,radius,count,roundness,angle,separation);
...
void main() {
    fragColor = rayMarcher((inverse(u_viewTransform) * vec3(((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), 1.0)).xy, ((v_uv - 0.5) * 2.0 * vec2(u_worldAspect, 1.0)), u_model3DTransform, u_sourceDim, u_lightSourceTransform, u_bkgTransform, u_camera3DTransform, u_colorMaterial, u_refractionIndex, u_fresnelStrength, u_chromaticAberration, u_colorFog, u_sourceColor, u_ambientColor, u_specular, u_backgroundStyle, u_radius, u_count, u_roundness, u_angle, u_separation);
}
