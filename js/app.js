...
import { clamp, compose3, compose4, cycle, decompose3, decompose4, fmt, fromHex, slider, stepOf, toHex, uniqueRuns, } from './controls.js';
...
function describeFilter(f) {
    ...
    if (extra.length)
        bits.push(`also reads ${extra.join(' and ')}`);
    if (!portsOf(f.id).length)
        bits.push('takes no image: it draws its own picture');
    return `${f.name}  (${f.id})\n${bits.join('\n')}`;
}
...
function addImageNode(into) {
    ...
    if (into)
        connect(id, into.node, into.port);
    draw();
...
function addNode(filterId, at) {
    ...
    if (ports.length)
        connect(tail ? tail.from : 'src', id, ports[0]);
...
function inView(id) {
    const el = $('nodes').querySelector(`.node[data-id="${id}"]`);
    if (!el)
        return true;
...
function removeNode(id) {
    ...
    if (upstream) {
        for (const d of downstream)
            connect(upstream.from, d.to, d.port);
    }
    chain.nodes.delete(id);
    if (chain.selected === id)
        select(null);
...
function nodeGraph(id, seen) {
    ...
    if (chain.nodes.get(id)?.kind === 'image')
        return { input: id };
    ...
    for (const port of portsOf(node.filter)) {
        ...
        if (link)
            inputs[port] = nodeGraph(link.from, seen);
    }
    seen.delete(id);
...
function loadChain(spec) {
    ...
    const place = (node, column) => {
        if (!isFilterGraph(node))
            return feed; // a leaf reads the chain so far
        const id = `n${chain.next++}`;
...
function ensureEnds() {
    ...
    if (!chain.nodes.has('out')) {
        ...
        if (!chain.links.some((l) => l.to === 'out'))
            connect('src', 'out', 'source');
    }
    ...
    if (!out || out.kind !== 'output' || out.moved)
        return;
    const last = chain.links.find((l) => l.to === 'out');
...
function nodeEl(node) {
    ...
    for (const port of portsOfNode(node))
        ports.appendChild(portEl(node, port, 'in'));
    if (node.kind !== 'output')
        ports.appendChild(portEl(node, 'out', 'out'));
...
function anchor(nodeId, port, dir) {
    const dot = $('nodes').querySelector(`.dot[data-node="${nodeId}"][data-port="${port}"][data-dir="${dir}"]`);
    ...
    let x = 0;
    let y = 0;
    for (let el = dot; el && el !== root; el = el.offsetParent) {
        x += el.offsetLeft;
        y += el.offsetTop;
    }
    return {
...
function startDragNode(e, node, el) {
    ...
    if (target.classList.contains('dot') || target.classList.contains('remove'))
        return;
    e.stopPropagation();
...
function startWire(e, nodeId, port, dir) {
    e.stopPropagation();
    e.preventDefault();
    ...
    const move = (ev) => drawWires({ a, b: graphPoint(ev) });
    const up = (ev) => {
        window.removeEventListener('pointermove', move);
        window.removeEventListener('pointerup', up);
        const target = document.elementFromPoint(ev.clientX, ev.clientY);
        const dot = target?.classList.contains('dot') ? target : null;
        const onNode = target?.closest('.node') ?? null;
        ...
        if (dot && to && toPort && dot.dataset.dir === 'in' && to !== origin.node) {
            ...
            if (free)
                connect(origin.node, onNode.dataset.id, free);
        }
        draw();
        render();
    };
...
function renderControls() {
    ...
    if (f.fidelity && f.fidelity !== 'extracted')
        bits.push(f.fidelity);
    ...
    for (const p of own)
        box.appendChild(control(p, node));
    if (engine.length) {
...
const asNumber = (v) => (typeof v === 'number' ? v : 0);
const asVector = (v) => (Array.isArray(v) ? v : []);
const asMatrix = (v) => Array.isArray(v) && Array.isArray(v[0]) ? v : null;
const asPalette = (v) => Array.isArray(v) && Array.isArray(v[0]) ? v : [];
function control(p, node) {
...
async function bindSources(gpu) {
    ...
    await onGpu(() => {
        ...
        for (const n of loaded) {
            const b = n.bitmap;
            if (b)
                gpu.set_input([n.id], b.data, b.width, b.height);
        }
    });
    bound = key;
}
function render() {
    if (!state.preview || !state.gpu)
        return;
    clearTimeout(timer);
    timer = setTimeout(doRender, 60); // debounce slider drags
}
async function doRender() {
    const gpu = state.gpu;
    if (!state.preview || !gpu)
        return;
    const seq = ++state.seq;
    ...
    if (!graph) {
        paint($('preview'), width, height, data);
    ...
    try {
        ...
        const out = await onGpu(() => (seq === state.seq
            ...
            : null));
        if (out === null || seq !== state.seq)
            return;
        paint($('preview'), width, height, out);
        $('preview').hidden = false;
        $('dropzone').hidden = true;
...
async function probeInert() {
    ...
    if (!node || !gpu || !spec || !state.preview)
        return;
    ...
    const draw = async (g) => onGpu(() => (run === state.probe
...
function otherValue(p, current) {
    ...
    if (p.type === 'bool')
        return typeof current === 'number' ? (current ? 0 : 1) : 1;
...
function markInert() {
    ...
    for (const el of $('params').querySelectorAll('.param[data-param]')) {
...
async function thumbnails() {
    ...
    for (const node of [...chain.nodes.values()]) {
        if (run !== state.thumbs)
            return;
        const canvas = $('nodes').querySelector(`canvas[data-thumb="${node.id}"]`);
        if (!canvas)
            continue;
        if (node.kind === 'image') {
            ...
            const b = node.bitmap;
            if (b)
                paint(canvas, b.width, b.height, b.data);
            continue;
        }
        ...
        if (painted.get(node.id) === key && canvas.width === tw)
            continue;
        let out = small;
        if (upto) {
            try {
...
function downscale(data, w, h, tw, th) {
...
function paint(canvas, width, height, rgba) {
    canvas.width = width;
...
async function readImage(file, limit = 1400) {
...
async function loadFile(file) {
    ...
    state.sourceId += 1;
    paint($('original'), image.width, image.height, image.data);
    paint($('preview'), image.width, image.height, image.data);
    $('dropzone').hidden = true;
    $('preview').hidden = false;
    render();
}
...
async function start() {
    ...
    state.filters = JSON.parse(catalog());
    ...
    $('imageInput').onchange = async (e) => {
        ...
        pickingFor = null;
        if (!file || !node || node.kind !== 'image' || !file.type.startsWith('image/'))
            return;
    ...
    graph.addEventListener('pointerdown', (e) => {
        const target = e.target;
        if (target.closest('.node') || target.closest('.graphbar'))
            return;
