...
vec3 sphereMap(vec3 dir, vec2 sourceBkgDim) {
    ...
    float nX = 2.0;
    float nY = 1.0;
    return vec3(-alpha/PI*0.5*nX, 0.5+nY*beta/PI, 1.0);
}
...
vec3 backgroundForReflection(vec3 dir, vec2 sourceBkgDim, int backgroundMode) {
        ...
        else if (backgroundMode==3) return boxMap(dir, sourceBkgDim); // simplified compared to Glitch Lab because of all the extra variables for a not great result
//        {
