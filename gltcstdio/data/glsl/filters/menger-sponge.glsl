...
vec3 hueRotate(vec3 c, float a) {
    ...
    return c*cosA + cross(k, c)*sin(a) + k*dot(k, c)*(1.0 - cosA);
        ...
        vec4 mengerSponge(vec2 uv, vec2 outPos, int iterations, mat4 model3DTransform, mat4 camera3DTransform, mat4 lightSourceTransform, mat4 internal3DTransform,
                vec2 sourceDim, vec2 sourceBkgDim, int sourceBkg_specified, int mode, int internalIter,
                float colorScheme, float thickness, float variability, float randomSeed,
                vec4 colorSource, vec4 colorAmbient, vec4 colorFog, vec4 colorGlow,
                vec2[11] params, float backgroundStyle, int backgroundMode,
                float specular, float glow, float fog, float gamma) {
